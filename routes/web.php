...
Route::group(['middleware' => ['redirect','install', 'limit.exceeded']], function () {
    ...
    Route::get('2fa/enable', 'Admin\helpdesk\Google2FAController@enableTwoFactor');
    Route::get('2fa/disable/{userId?}', 'Admin\helpdesk\Google2FAController@disableTwoFactor');
...
// NOTE FROM AVINASH: there no such concept as group in faveo. Commenting to avoid future headaches
//    Route::resource('groups', 'Admin\helpdesk\GroupController', ['except' => 'show']); // for group module, for CRUD
    // show method has been renamed
    ...
    // NOTE: delete functionality has been remove in v2.1.2
    // it must be converted into vue with delete functionality
    Route::delete('plugin/delete/{slug}', ['as' => 'delete.plugin', 'uses' => 'Common\SettingsController@DeletePlugin']);
    ...
    Route::get('language/iso-code', ['as' => 'add-language.iso-code', 'uses' => 'Admin\helpdesk\LanguageController@getIsocode']);

    //Routes for  delete language package
    Route::delete('delete-language/{lang}', ['as' => 'lang.delete', 'uses' => 'Admin\helpdesk\LanguageController@deleteLanguage']);
    ...
    Route::get('agent/type/search', ['as' => 'agent.type.search', 'uses' => 'Admin\helpdesk\AgentController@findType']);

    Route::get('user-options', ['as' => 'users-options', 'uses' => 'Admin\helpdesk\SettingsController@getUserOptions']);


    Route::post('user-options', ['as' => 'users-options', 'uses' => 'Admin\helpdesk\SettingsController@postUserOptions']);
    ...
    Route::get('org/name/search', ['as' => 'org.name.search', 'uses' => 'Agent\helpdesk\UserController@find']);
    ...
    Route::post('helpsection/mail', ['as' => 'helpsection.mail', 'uses' => 'Admin\helpdesk\HelpsectionController@sentmail']);
    ...
    Route::get('/search/department/autofill', ['as' => 'search.department.autofill', 'uses' => 'Agent\helpdesk\TicketController@departmentAutofill']);
    //Route to seach ticket
    Route::get('ticket-search/', ['as' => 'ticket-search', 'uses' => 'Agent\helpdesk\TicketController@autoSearchticket']);
    ...
    Route::post('/newticket/post', ['as' => 'post.newticket', 'uses' => 'Agent\helpdesk\TicketController@post_newticket'])
        ->middleware('role.agent'); /*  Post Create New Ticket */

    Route::post('/newbatchticket/post', ['as' => 'post.newbatchticket', 'uses' => 'Agent\helpdesk\TicketController@post_newbatchticket'])
        ->middleware('role.agent'); /*  Post Create New Ticket */
    ...
    Route::get('canned/get-departmens/{id}', ['as' => 'get-canned-department', 'uses' => 'Agent\helpdesk\CannedController@getCannedDepartments']);
    ...
    Route::get('/multi-head-org/autofill/{org_id}', ['as' => 'post.org-head.autofill', 'uses' => 'Agent\helpdesk\OrganizationController@getHrdname']);
    Route::get('/micro-org-user/list/{dept}', ['as' => 'micro.org.user.show', 'uses' => 'Agent\helpdesk\OrganizationController@microOrgUserList']);
    ...
    Route::get('department/autofill', ['as' => 'post.department.autofill', 'uses' => 'Agent\helpdesk\TicketController@departmentAutofill']);
     ...
     * Label
     */
    Route::get('all-labels', ['as' => 'all-labels', 'uses' => 'Admin\helpdesk\Label\LabelController@getLabels']);
    ...
    // KAS
    // Get trouble tickets of a customer
    Route::get('api/agent/troubleTicket', 'Agent\helpdesk\TicketsView\TicketListController@listTroubleTicket');

    Route::get('api/agent/edit-mode-ticket-details/{ticketId}', 'Agent\helpdesk\TicketsView\TicketTimelineController@ticketDetailsInEditMode')->name('agent.edit-mode-ticket-details');
    ...
    //creates/updates ticket through mail
//    Route::get('/fetchmails', 'Common\Mail\FetchMailController@fetchMail')->name('fetchmails');
...
// ===================================================================================================================
    ...
    Route::get('delete/emailpopup/{id}', 'Common\Mail\ApiMailController@destroy');
    ...
    Route::get('get-ticket-tag', ['as' => 'ticket-tag.get', 'uses' => 'Agent\helpdesk\Filter\TagController@getTagForTicket']);
    ...
    Route::get('source/location/search', ['as' => 'source.location.search', 'uses' => 'Admin\helpdesk\Source\SourceController@findLocation']);
...
//===================================================================================
    Route::group(['middleware' => 'auth'], function () {
        ...
        Route::get('/client/manager/user/create', ['as' => 'client.manager.usercreate', 'uses' => 'Client\helpdesk\GuestController@managerCreateUser']);
        Route::post('client/user/create', 'Client\helpdesk\GuestController@storeUser');

        //attachment
        Route::get('client/docs_list/{org_id}', ['as' => 'client.org.docs_lists', 'uses' => 'Client\helpdesk\GuestController@managerLicensesDocs']);
        Route::get('/Client/download/{file_name}', ['as' => 'client.org.download.docs', 'uses' => 'Client\helpdesk\GuestController@clientDownloadDocs']);


        Route::get('/client/manager/user/view/{user_id}', ['as' => 'client.manager.userview', 'uses' => 'Client\helpdesk\GuestController@managerViewUser']);
    ...
    Route::group(['middleware' => ['kbaccess']], function () {
        ...
        Route::post('kb/status/index', ['as' => 'kb.status.index', 'uses' => 'Agent\kb\SettingsController@kbstatusIndex']);
    ...
    Route::get('delete/cannedpopup/{id}', ['as' => 'helper.canned.delete', 'uses' => 'Agent\helpdesk\CannedController@destroy']);
...
// we have removed thread pdf generation. Same thing can be achieved by ticket pdf
Route::get('genereate-pdf/{threadid}', ['as' => 'thread.pdf', 'uses' => 'Agent\helpdesk\TicketController@pdfThread']);

/**
 * @since v4.0.0
 * @internal all these were commented routes blocks moved from above for better removal
 */

//            NOTE FROM AVINASH: commenting in the assumption that they are not being used
//            Route::get('/search/department/autofill', ['as' => 'search.department.autofill', 'uses' => 'Agent\helpdesk\TicketController@departmentAutofill']);
//            Route::get('/search/ticket-type/autofill', ['as' => 'search.ticket-type.autofill', 'uses' => 'Agent\helpdesk\TicketController@helptopicType']);
...
//            Route::get('thread/auto/{id}', 'Agent\helpdesk\TicketController@autosearch');
    ...
    // NOTE FROM AVINASH: duplicate route. Commenting to avoid future blunders
//        Route::post('user-chart-range/{id}/{team}/{date1}/{date2}', ['as' => 'post.user.chart', 'uses' => 'Agent\helpdesk\DashboardController@userChartData']);
//        Route::get('user-agen/{id}', 'Agent\helpdesk\DashboardController@userChartData');
//        Route::resource('user', 'Agent\helpdesk\UserController'); /* User router is used to control the CRUD of user */
//        Route::get('org-user-list/{org_id}', ['as' => 'org.user.list', 'uses' => 'Agent\helpdesk\OrganizationController@getOrgUserList']);
//        Route::get('org-dept-list/{org_id}', ['as' => 'org.dept.list', 'uses' => 'Agent\helpdesk\OrganizationController@getOrgDeptList']);
//        Route::get('org-dept-user-list/{org_id}/{deptid}', ['as' => 'org.dept.user.list', 'uses' => 'Agent\helpdesk\OrganizationController@getOrgDeptUserList']);
//        Route::post('delete-org-dept', ['as' => 'delete.org.dept', 'uses' => 'Agent\helpdesk\OrganizationController@deleteOrgDept']);
//        Route::get('home', ['as' => 'home', 'uses' => 'Client\helpdesk\WelcomepageController@index']); //guest layout


//          NOTE FROM AVINASH: these routes are not in use anymore. Commenting to avoid future headaches
//            Route::get('clientpanel/user/password', ['as' => 'clientpanel.user.changepassword', 'uses' => 'Client\helpdesk\GuestController@randomPassword']);
//            Route::post('clientpanel/user/changepassword/{id}', ['as' => 'clientpanel.user.post.changepassword', 'uses' => 'Client\helpdesk\GuestController@randomPostPassword']);
//            Route::post('clientpanel/user/restore/{id}', ['as' => 'clientpanel.user.restore', 'uses' => 'Client\helpdesk\GuestController@restoreUser']);
//            Route::get('clientpanel/user-agen/{id}', ['as' => 'clientpanel.user-agen', 'uses' => 'Client\helpdesk\GuestController@userChartData']);
//            Route::post('clientpanel/select_all', ['as' => 'clientpanel.select_all', 'uses' => 'Client\helpdesk\GuestController@select_all']);
...
//            Route::get('postcheck', 'Client\helpdesk\GuestController@PostCheckTicket');
    ...
    // NOTE FROM AVINASH: duplicate routes, probably not in use. Commenting it for future blunders
