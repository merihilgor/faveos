...
@section('content')
<!-- open a form -->
{!! Form::open(['action' => 'Admin\helpdesk\HelptopicController@store', 'method' => 'post','id'=>'Form']) !!}
   @if(Session::has('errors'))
        ...
        <div class="alert alert-danger alert-dismissable">
            ...
            @endif
            @if($errors->first('linked_departments'))
            <li class="error-message-padding">{!! $errors->first('department', ':message') !!}</li>
            @endif
...
<div class="box box-primary">
    <div class="box-header with-border">
        <h3 class="box-title">{{Lang::get('lang.create_new_help_topic')}}</h3>
    </div>
    <div class="box-body">
            ...
            <div class="box-body table-responsive no-padding" style="overflow:hidden">
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 form-group {{ $errors->has('linked_department') ? 'has-error' : '' }}">
                        {!! Form::label('linked_departments',Lang::get('lang.linked_department')) !!} <span class="text-red"> *</span>&nbsp;&nbsp;<a href="#" data-toggle="tooltip" data-placement="bottom" title="{!! Lang::get('lang.linked-department-tooltip-message') !!}"><i class="fa fa-question-circle" style="padding: 0px;"></i></a>
                        {!! Form::select('linked_departments[]', [Lang::get('lang.departments')=>$departments->pluck('name','id')->toArray()],null,['multiple'=>true,'class'=>"form-control select2" ,'id'=>"primary_department",'required','style'=>"width:100%"]) !!}
                    ...
                    <div class="col-md-12 form-group {{ $errors->has('department') ? 'has-error' : '' }}" id="default-department-block" style="display: block;">
                        <div class="form-group {{ $errors->has('linked_department') ?  'has-error' : '' }}">
                            {!! Form::label('department',Lang::get('lang.default_department')) !!}<span class="text-red"> *</span>&nbsp;&nbsp;<a href="#" data-toggle="tooltip" title="{!! Lang::get('lang.default-department-tooltip-message') !!}"><i class="fa fa-question-circle" style="padding: 0px;"></i></a>
                        ...
                        <?php $check_linking_status = \App\Model\helpdesk\Settings\CommonSettings::where('option_name', '=', 'helptopic_link_with_type')->select('status')->first(); ?>
                        

                                   @if ($check_linking_status->status != 0)
                                        <div class="row">
                                            <div class="col-xs-8 form-group {{ $errors->has('type') ? 'has-error' : '' }}">
                                                {!! Form::label('type',Lang::get('lang.type')) !!} 
                                                {!! Form::select('ticket_type[]', [],null,['multiple'=>'true','class'=>"form-control select2" ,'id'=>'ticket_type','style'=>'width: 100%;' ]) !!}
                ...
                <div class="row">
                    ...
                    <div class="col-md-12">
                        <div class="form-group">
                            ...
                            {!! Form::textarea('internal_notes',null,['class' => 'form-control','id'=>'ckeditor','size' => '10x5']) !!}
                        </div>
                    </div>
                </div>
            </div>
...
<script type="text/javascript">    
    ...
    $(function() {
        ...
        @if(Input::old('department', false))
            old = {!! Input::old('department', false) !!}
        @endif
        if (old == 0) {
