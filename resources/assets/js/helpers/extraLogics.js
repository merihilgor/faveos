...
export const extractOnlyId = (array) => {
	// console.log(array, "extract only id")
	if (array.length > 0) {
...
export const fetchNameAsPerId = (value, listdata) => {
	...
	finalArray = listdata.filter(elements => {
		// console.log(elements.id, "filter");
		return value.includes(elements.id.toString());
	});
	// console.log(finalArray, "filter array");
	return finalArray;
}
...
export const getValueFromNestedArray = (editDataValues, valueToBeMatched, requiredValueFromArray) => {
	var result = "";
	console.log(editDataValues, valueToBeMatched, requiredValueFromArray)
...
export const FORM_BUILDER_UTILS = {
	getApiInfoObj: (string) => {
		if (!string) return {};
		const apiObj = {};
		const splitByPart = string.split(';;');
		for (const info of splitByPart) {
			const splitByKeyValue = info.split(':=');
			apiObj[splitByKeyValue[0]] = splitByKeyValue[1]
		}
		return apiObj;
	},

	getApiInfoString: (object) => {
		if (!object) return '';
		let apiStringArr = [];
		for (const key in object) {
			apiStringArr.push(key + ':=' + object[key]);
		}
		return apiStringArr.join(';;');
	}
}