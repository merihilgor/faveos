<template>
	...
	<modal v-if="showModal" :showModal="showModal" :onClose="onClose" :containerStyle="containerStyle">
		...
		<div slot="title">
				...
				<h4>{{lang('change_status')}}</h4>
		</div>

		<div slot="fields" v-if="!loading">

			<p id="change" v-if="ticketIds.length > 0">{{'Are you sure you want to set status to ' +status.name+ '?'}}</p>

			<p id="change" v-else>{{lang('select-ticket')}}</p>
