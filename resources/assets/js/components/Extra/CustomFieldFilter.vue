<template>
	...
	<div id="filter-div" class="btn-group box-tools pull-right">
			
		<button type="button" class="btn btn-box-tool dropdown-toggle" data-toggle="dropdown" id="labels-button"
			@click="getFields()">

			<i class="fa fa-plus"> </i> {{trans('add_custom_field')}} 
		</button>
	
		<ul id="filter-dropdown" class="dropdown-menu filter_menu" @click="$event.stopPropagation()">

			<li v-if="loading" class="field_loader">
			...
			<template v-if="!loading">
				...
				<li v-for="field in filteredList" :class="['fields',{'active' : selectedFields.map(a => a.id).includes(field.id)}]" 
					@click="onClick(field)">
...
<script>
	...
	export default{
		...
		created() {
			...
			window.eventHub.$on('clearCustomFields',this.clearFields)
		},
		...
		methods :{
			...
			getFields(){
				...
				axios.get('/service-desk/api/asset-custom-field-list').then(res=>{
...
<style scoped>
	...
	.filter_div{	margin: 6px 3px 3px 3px; }

	.fields:hover,.active{ background: #eee; }
	
	.fields,#field_title{ cursor: pointer; }

	.field,#field_title{ margin : 3px; }

	.fields{ margin: 2px; padding: 3px; }

	.filter_menu{ width: auto; max-height: 400px; overflow-y: auto; overflow-x: hidden; padding: 5px;}
