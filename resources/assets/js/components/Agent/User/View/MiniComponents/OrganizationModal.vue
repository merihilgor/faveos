	...
	<div> 
		...
		<modal v-if="showModal" :showModal="showModal" :onClose="onClose" :containerStyle="containerStyle">
			...
			<div slot="fields" v-if="title === 'remove' && !loading">
				...
				<h5 id="H5">{{lang('are_you_sure_you_want_to_remove')}}</h5>

			</div>

			<!-- assign existing organization -->

			<div slot="fields" v-if="title === 'assign' && !loading" id="dynamic_select">

				<div class="row">

					<dynamic-select :label="lang('organizations')" :multiple="true" name="organizations" 
						classname="col-xs-12" apiEndpoint="/api/dependency/organizations" :value="organizations" 
					...
					<dynamic-select v-if="deptCondition" :label="lang('organization_department')" :multiple="false" name="org_dept" 
						 classname="col-xs-12" :apiEndpoint="org_dept_api" :value="org_dept" 
	...
	export default {
		...
		data(){
			...
			return {
				...
				iconClass : this.title === 'assign' ? 'fa fa-hand-o-right' : 'fa fa-unlink',
			}
		},
