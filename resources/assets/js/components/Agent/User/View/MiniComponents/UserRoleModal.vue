	...
	<div> 
		...
		<modal v-if="showModal" :showModal="showModal" :onClose="onClose" :containerStyle="containerStyle">
			...
			<div slot="title">
				...
				<h4>{{lang('role_change')}} {{role}} to {{role_to}}</h4>
			
			</div>

			<div slot="fields" v-if="role_to === 'user' && !loading">

				<h5 id="H5">{{lang('changing_role_to_user')}} <b>{{user.user_name ? user.user_name : user.email }}</b>. {{lang('do_you_want_to_proceed_to_change_their_role_to_user')}} </h5>

			</div>

			<div slot="fields" v-if="!loading && role_to !== 'user'" id="dynamic_select">

				<div class="row">

					<dynamic-select :label="lang('departments')" :multiple="true" name="departments" 
						classname="col-xs-12" apiEndpoint="/api/dependency/departments" :value="departments" 
