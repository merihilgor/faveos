<template>
	...
	<modal v-if="showModal" :showModal="showModal" :onClose="onClose" :containerStyle="containerStyle">
		...
		<div slot="title">
			...
			<h4>{{'Are you sure you want to set status to ' +status.name+ '?'}}</h4>
		</div>
			...
			<div class="row">
				...
				<div class="form-group col-md-12">
					...
					<text-field :label="lang(status.comment ? 'leave_comment_status_change_required' : 'leave_comment_status_change_optional')" :value="commnetValue" type="textarea" name="commnetValue" 
					 	:onChange="onChange" :required="status.comment ? true : false" classname="col-xs-12">
						
					</text-field>
				</div>

				<div class="form-group col-md-12" id="textchange">

					<label class="label_align1 col-xs-12">
