<template>
  <div class="box box-primary dashboard-widget-box">
    <div class="box-header with-border">
      ...
      <h3 class="box-title">{{lang('recent_activities')}}</h3>
      <i class="fa fa-refresh pull-right" aria-hidden="true" @click="refreshData()" :title="lang('refresh')"></i>
    </div>
    <div class="box-body scrollable-area" v-if="notificationData.length > 0">
      <ul class="products-list product-list-in-box">
        <li class="item" v-for="activity in notificationData" :key="activity.id">
          ...
          <div class="product-info" style="padding-top: 1rem;">
            ...
            <a v-else class="product-title" :href="activity.url" target="_blank">{{ activity.requester.changed_by_first_name + ' ' + activity.requester.changed_by_last_name + ' ' + activity.message}}</a>
            <strong class="pull-right">{{activity.created_at}}</strong>
          </div>
        </li>
