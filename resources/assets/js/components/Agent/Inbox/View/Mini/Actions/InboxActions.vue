<template>
	...
	<div id="inbox-actions" v-if="getTicketActions">
		...
		<div class="nav-tabs-custom no-box-shadow ">

			<ul class="nav nav-tabs">

			  <li v-for="section in tabs" :class="{ active: category === section.id }">

				 <a id="actions_tab" data-toggle="tab" @click="associates(section.id)">
			...
			<div class="tab-content" id="inbox-actions-tab">
			  ...
			  <div v-if="!loading" class="active tab-pane" id="activity">
		...
		data() {
			...
			return {
				...
				tabs:[
					 ...
					 {id : 'reply', title : 'reply', icon : 'fa fa-reply-all'},
				  
					 {id : 'internal', title : 'internal_notes', icon : 'fa fa-file-text' }
				],
			...
			checkActions(actions) {
				...
				if(actions){
					...
					if(!this.tabs.some(tab => tab.id === "progress") && actions.view_approval_progress){
						...
						this.tabs.push({id : 'progress', title : 'approval_progress', icon : 'fa fa-tasks'})
					
					} 

					if(!this.tabs.some(tab => tab.id === "time") && actions.time_track_enabled){

						this.tabs.push({id : 'time', title : 'time-track', icon : 'fa fa-clock-o'})
					}
				}
			},
