<template>
	...
	<div class="timeline_details">
        ...
        <div v-if="data.sla_plan" class="col-md-6 info-row">
				...
				<div v-if="data.assignee" class="col-md-6 info-row">
					...
					<div class="col-md-6">
						...
						<a v-tooltip="data.assignee" target="_blank" :href="data.assignee.href">{{subString(data.assignee.name)}}</a>
					</div>
				</div>
        ...
        <div v-if="data.organizations" class="col-md-6 info-row">

          <div class="col-md-6"><label>{{lang('organization')}}</label></div>

          <div class="col-md-6">
            <div v-for="organization in data.organizations" >
              <a target="_blank" :href="organization.href">{{subString(organization.name)}}</a>
            </div>
          </div>
        ...
        <template v-if="timelineData.custom_field_values && timelineData.custom_field_values.length > 0">
					...
					<div v-for="customField in timelineData.custom_field_values" class="col-md-6 info-row">
				...
				<div id="timeline-meta-list-box">{{timelineMetaListBoxMounted()}}</div>
			 </div>
		 </div>
	</div>
</template>
