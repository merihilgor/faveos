<template>
	...
	<div>
		...
		<div v-if="getTicketActions && !getTicketActions.block_ticket_actions" 
			...
			<button type="button" class="btn btn-sm btn-default" @click="showEditModal = true">
			...
			<button  v-if="getTicketActions.assign"  type="button" class="btn btn-sm btn-default" @click="showAssignModal = true">
			...
			<button  v-if="getTicketActions.change_duedate"  type="button" class="btn btn-sm btn-default" 
				@click="showDueModal = true">
				
				<i  class="fa fa-calendar"> </i> {{lang('change_due_date')}}
			</button>
			...
			<button  v-if="getTicketActions.surrender"  type="button" class="btn btn-sm btn-default" 
				@click="showSurrenderModal = true">
				
				<i  class="fa fa-arrows-alt"> </i> {{lang('surrender')}}
			</button>
			...
			<transition name="modal">
		 	...
		 	<button  v-if="getTicketActions.transfer"  type="button" class="btn btn-sm btn-default" 
				@click="showDeptModal = true">
				
				<i  class="fa fa-hand-o-right"> </i> {{lang('change_department')}}
			</button>
		   ...
		   <ticket-tag :ticketObj="data" :reloadDetails="reloadData"></ticket-tag>
		    ...
		    <a :href="basePath() + '/ticket/print/' + id" target="_blank" class="btn btn-sm btn-default">
		    	
		    	<i class="fa fa-file-pdf-o"> </i> {{ lang('generate_pdf') }}
		    </a>

		   <div class="btn-group dropdown">

				<button  class="btn btn-sm btn-default" id="status" @click="getStatusList()" data-toggle="dropdown">
			...
			<button  v-if="getTicketActions.allowed_enforce_approval_workflow"  type="button" class="btn btn-sm btn-default" 
				@click="showApprovalModal = true">
				
				<i  class="fa fa-pause"> </i> {{lang('apply_approval_workflow')}}
			</button>
			...
			<button  type="button" class="btn btn-sm btn-default"  @click="showForwardModal = true">
				
				<i  class="glyphicon glyphicon-forward"> </i> {{lang('forward_ticket')}}
			</button>
...
<style scoped>
	...
	.ticket-actions .btn { margin-bottom: 2px; }

	.ticket-actions .btn-group .btn { margin-bottom: 2px; }
</style>

<style>
	
	#timeline-action-bar .btn { margin-bottom: 2px; }
