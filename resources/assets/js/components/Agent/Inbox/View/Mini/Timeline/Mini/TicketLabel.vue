<template>
	...
	<div id="labels-div" class="btn-group">
			...
			<button type="button" class="btn btn-sm btn-default dropdown-toggle" data-toggle="dropdown" id="labels-button"
				@click="getLabels(search)">

				<i class="fa fa-lastfm"> </i> {{lang('labels')}} 
		...
		<ul id="label-dropdown" class="dropdown-menu label_menu" @click="$event.stopPropagation()">
			...
			<template v-if="!loading">
				...
				<li>
					...
					<div class="form-group has-feedback has-search label_div">
				    
				    <span class="glyphicon glyphicon-search form-control-feedback"></span>
				    
				    <input type="text" class="form-control" name="label" :placeholder="lang('search')" v-model="search" 
							@input="getLabels(search)">
				  </div>
				</li>
				
				<li v-for="label in labels" :class="['labels',{'active' : selectedLabels.includes(label.name)}]" 
					...
					<i v-if="selectedLabels.includes(label.name)" class="fa fa-times"
	...
	export default{
		...
		methods :{
			...
			onCancel(){
				...
				var element = document.getElementById("labels-div");
			...
			getLabels(search){
				...
				axios.get('/api/dependency/labels?search-query='+search).then(res=>{
...
<style scoped>
	...
	#labels-div .btn { margin-bottom: 2px; }
