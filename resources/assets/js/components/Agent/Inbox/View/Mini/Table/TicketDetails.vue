<template>
	...
	<tr :class="{'unanswered': ticket.isanswered == 0, 'active_ticket' : getStoredTicketId == ticket.id}" v-bs-popover>
		...
		<td class="td-ticket-status hideForList">
			...
			<span class="overdue subject_style" v-show="ticket.is_overdue">{{ lang('overdue') }}</span>

			<span class="due-today subject_style" v-show="ticket.due_today">{{ lang('due_today') }}</span>
		</td> 
