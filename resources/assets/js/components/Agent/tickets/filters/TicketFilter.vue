...
.inbox-new #filter-trigger-button {
...
<template>
	<div class="inline" v-bind:class="{'pull-right':isRtl,'pull-left':!isRtl}">
		<custom-loader v-if="loading"></custom-loader>
		 <div class="flex pull-right" :class="{'tic_filter': iconClass != 'fa fa-cogs'}">
			 <button type="button" class="btn btn-primary ticket-filter" id="filter-trigger-button" v-tooltip="lang(tooltipText)"
			 	@click="openFilter()" v-bind:style="{background:tableHeader+'!important',color:'#fff'}" ><i :class="iconClass"></i></button>
	 	 </div>

		 <div id="filter-container" class="box" style="clear:both;" v-if="showFilter">
	   	   <div class="box-header with-border">
            <h4 class="box-title">Apply Filter</h4>

						<!-- Custom Field Filter -->
            <custom-field-filter :selectedFilters="customFieldFilters" v-on:custom="getCustomFields"></custom-field-filter>

						<save-filter v-if="showFilterPopUp" :filterName="filterName" :filterValues="filter" :filterObjects="filterObjects" :filterId="selectedFilter"
								:toggleFilterPopUp="toggleFilterPopUp" :mode="filterMode" :intitialDepartments="departments" :initialAgents="agents" :postFilterSaveAction="applyFilter" :dashboard-config="dashboardConfig">
						</save-filter>
	        ...
	        <div class="box-footer">
								...
								<button id="delete-filter" v-if="selectedFilter !== 0" type="button" @click = "saveFilter('delete')" class="btn btn-danger">
								...
								<button id="close-filter" type="button" @click = "openFilter()" class="btn btn-default">
									<i class="glyphicon glyphicon-remove" aria-hidden="true"></i>&nbsp;{{lang('close')}}
								</button>
...
<script>
	...
	export default {
		...
		props: {
			...
			/**
			 * Filter button tooltip text
			 */
			tooltipText: {
				type: String,
				default: 'filter_tickets'
			},
      ...
      this.handleTooltip();
		...
		methods:{
			...
			resetDashboardConfig() {
				...
				this.dashboardConfig.iconClass = { "icon_class": "fa fa-ticket" };
