<template>
  ...
  <modal :showModal="true" :onClose="onClose">
    ...
    <div slot="title">
      ...
      <header v-if="mode !== 'delete'">{{lang('save_filter')}}</header>
      <h4 v-else>{{lang('are_you_sure_you_want_to_delete')}}</h4>
      <alert :componentName="this.$options.name"/>
    </div>



    <div v-if="mode !== 'delete'" id="share-filter-box" slot="fields" class="save-filter-modal-body">
    ...
    <div v-else id="share-filter-box" class="margin-bottom-20" slot="fields">
      <h4>You are about to delete a filter with name <strong>{{filterName}}</strong>
        created by you. Please confirm</h4>
    </div>


    <div slot="controls">
      <button v-if="showSave()" type="button" @click = "saveFilter('update')" class="btn btn-primary"><i class="fa fa-refresh" aria-hidden="true"></i>&nbsp;{{lang('update')}}</button>
      <button v-if="showSaveAsNew()" type="button" @click = "saveFilter('create')" class="btn btn-primary"><i class="fa fa-save" aria-hidden="true"></i>&nbsp;{{lang('save_as_new')}}</button>
      <button v-if="showDelete()" type="button" @click = "deleteFilter()" class="btn btn-danger"><i class="fa fa-trash" aria-hidden="true"></i>&nbsp;{{lang('delete')}}</button>
    </div>
