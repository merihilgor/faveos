<template>
	...
	<div id="holidayTable">
		<div class="flex">
			...
			<button class="btn btn-primary margin" @click="showModal = true"><span class="glyphicon glyphicon-plus"> </span> {{lang('add_new')}}</button>
			</div>
			<!-- alert component -->
					...
					<tr v-for="(day,index) in data" class="head">
						...
						<td>
							...
							<a id="delete" @click="deleteHoliday(index)" href="javascript:void(0)">
								<span title="Delete Holiday" class="btn btn-danger"><i class="fa fa-trash"></i></span>
							</a>
						</td>
			...
			<table v-else class="table">
				<h4 style="text-align:center">{{lang('no-data-to-show')}}</h4>
			</table>
			<!-- popup -->
