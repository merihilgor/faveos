<template>
	<div> 
		...
		<modal v-if="showModal" :showModal="showModal" :onClose="onClose" :containerStyle="containerStyle" :addRow="addRow" :deleteRow="deleteRow">
			...
			<div v-if="deleteData === 'yes'" slot="fields">
			...
			<div v-if="deleteData !== 'yes'" slot="fields">
				<div class="row">
					<div class="form-group col-xs-12">
						...
						:onChange="onChange" :required="true" format="MM-DD-YYYY" classname="col-xs-6" :clearable="false" :disabled="false" :before="before" :after="after"></date-time-field>
					</div>
					<div class="form-group col-xs-12">		
						<text-field :label="lang('description')" :value="holiday_description" type="textarea" name="holiday_description"
						:onChange="onChange" :required="true" :length="35" classname="col-xs-12" :show-word-limit="true"></text-field>
					</div>
				</div>
			</div>

			<div slot="controls">
				<button type="button" @click = "onSubmit" :class="classname" ><i class="fa fa-save" aria-hidden="true"></i> {{lang(btnName)}}</button>
			</div>
