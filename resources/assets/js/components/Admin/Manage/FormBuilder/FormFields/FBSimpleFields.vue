<template>
  <div class="row form-field">
    ...
    <div class="col-md-6">
      <!-- text/number/date/email/file/api fields -->
      ...
      <!-- textarea field-->
      <textarea v-if="fieldData.type=='textarea'" :id="'long-text-input' + fieldData.id" class="form-control" rows="1" :placeholder="lang('long_text')" disabled />
    </div>
...
import FBFormFieldLabel from './FBFormFieldLabel';
...
export default {
  ...
  methods: {
    // returns `true` if field is text/number/date/email/file
    isSimpleField() {
      const simpleFields = ['text', 'number', 'date', 'email', 'file'];
