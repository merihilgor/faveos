<template>
  ...
  <div class="box-container" ref="stickyBox" :style="{ width: headerWidth }" :class="{ sticky: isSticky }">

    <div class="box-body" id="drag__body">

      <span class="dropdown pull-right form-group-list" v-if="formGroupList && formGroupList.length > 0 && getFormCategoryType !== 'user' && getFormCategoryType !== 'organisation'">
        ...
        <draggable-element :list="formGroupList" tag="ul" :group="{ name: 'faveo-form-builder', pull: 'clone', put: false }" :clone="cloneElement" class="dropdown-menu">
          <li v-for="item in formGroupList" v-bind:key="item.id">
            <a style="cursor: move" href="javascript:void(0)">{{ item.name }}</a>
          </li>
        </draggable-element>
      </span>
      ...
      <draggable-element class="form__menulsit" :list="getFormMenus" :group="{ name: 'faveo-form-builder', pull: 'clone', put: false }" :clone="cloneElement">
        <span v-for="element in getFormMenus" :key="element.id">
          <span :title="element.title" class="form__menu"><i :class="['fa', element.icon_class]" aria-hidden="true"></i></span>
        </span>
      </draggable-element>
