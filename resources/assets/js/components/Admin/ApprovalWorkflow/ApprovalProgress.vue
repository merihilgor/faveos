<template>
	<div id="approval-progress" class="appproval">
			...
			<div  v-if="!loading" id="approval-progress">
				...
				<div class="box-container margin_box" v-for="(workflow, workflowIndex) in workflowData" :key="'workflow'+workflowIndex" :id="'approval-progress-element-'+workflowIndex" :style="{'border-color': getIconStyle(workflow.status,'color')}">
					<div class="box-header with-border" :style="{'background-color': getIconStyle(workflow.status,'color')}">
						<h3 class="box-title approval_title">
							<span>{{workflow.name}}</span>
							<span :title="workflow.status" :class="['workflow-status', getIconStyle(workflow.status)]"></span>
						</h3>
					</div>
						<!--  levels -->
						<div class="levels">
							<div class="box-body approval-level" v-for="(level,levelIndex) in workflow.approval_levels" :id="'approval-level-'+levelIndex" :style="{opacity : getOpacity(level.is_active)}">
								<div class="box-container margin-0" :style="{'border-color': getIconStyle(level.status,'color')}">
									<div class="box-header with-border no-wrap flex" :style="{'background-color': getIconStyle(level.status,'color')}">
										<span class="box-title mb_5">
											{{level.name}}
										</span>&nbsp;&nbsp;
			...
			<modal v-if="showModal" :showModal="true" :onClose="() => showModal = false">
				...
				<div class="row margin-auto" slot="fields">
					...
					<h4 v-if="!loading" class="padding-left-15 font-size-14">{{lang(modalQuestion)}}</h4>
