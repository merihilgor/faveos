<template>
    <div class="box-body">
        ...
        <div :class="['row', 'protocol',{'protocol-settings': sending_status == 1}]">
            ...
            <!--send-->
            <static-select :label="lang('transfer_protocol')"  :elements="sending_protocols"
               name="sending_protocol" :value="sending_protocol" classname="col-xs-3" :onChange="onChange" :required="true">
            </static-select>

            <!--fetch , send in SMTP-->
            <text-field v-if="hasHost" :label="lang('host_name')" :value="sending_host" name="sending_host"
             classname="col-xs-3":onChange="onChange" :required="true">
            </text-field>

            <!--sending ports -->
            <text-field id="sending-port" v-if="shallPortBeVisible" :label="lang('port_number')" :value="sending_port" name="sending_port" classname="col-xs-3"
            ...
            <static-select id="sending-encryption" v-if="shallEncryptionBeVisible" :label="lang('encryption')"  :elements="sending_encryptions" name="sending_encryption"
               :value="sending_encryption" classname="col-xs-3" :onChange="onChange" :required="true">
            </static-select>

            <static-select id="version" v-if="shallVersionBeVisible" :label="lang('version')"  :elements="versions" name="version"
                           :value="version" classname="col-xs-3" :onChange="onChange" :required="true">
            </static-select>

            <text-field v-if="hasApiKey" :label="lang('api_key')" :value="api_key" name="api_key" classname="col-xs-3"
                        :onChange="onChange" :required="true"></text-field>

            <text-field v-if="hasSecret" :label="lang('secret')" :value="secret" name="secret" classname="col-xs-3"
                        :onChange="onChange" :required="true"></text-field>

            <text-field v-if="hasDomain" :label="lang('domain')" :value="domain" name="domain" classname="col-xs-3"
                        :onChange="onChange" :required="true"></text-field>

            <text-field v-if="hasRegion" :label="lang('region')" :value="region" name="region" classname="col-xs-3"
                        :onChange="onChange" :required="true"></text-field>
