  ...
  <div>
    ...
    <faveo-box :title="lang('email_settings')" v-if="hasDataPopulated">
      <faveo-box :title="lang('email_info')">
        <div class="row">
          ...
          <text-field label="Email" :value="email_address" type="email" name="email_address" :onChange="onChange"
            classname="col-xs-3" :required="true"></text-field>

          <!-- email address -->
          <text-field label="Username" :value="user_name" type="email" name="user_name" :onChange="onChange"
            classname="col-xs-3" :hint="lang('user_name_hint')"></text-field>

          <!-- email name -->
          <text-field label="Name" :value="email_name" name="email_name" :onChange="onChange" classname="col-xs-3"
          ...
          <text-field v-if="hasPassword" label="Password" type="password" :value="password" name="password"
            :onChange="onChange" classname="col-xs-3" :required="true"> </text-field>
      ...
      <faveo-box :title="lang('ticket_settings')">
        ...
        <div class="row">
          ...
          <dynamic-select :label="lang('department')" :multiple="false" name="department" 
            classname="col-xs-4" apiEndpoint="/api/dependency/departments" :value="department" :onChange="onChange">
          </dynamic-select>

          <!--help topics-->
          <dynamic-select :label="lang('help_topic')" :multiple="false" name="helptopic" 
            classname="col-xs-4" apiEndpoint="/api/dependency/help-topics" :value="helptopic" :onChange="onChange">
          </dynamic-select>

          <!--priority-->
          <dynamic-select :label="lang('priority')" :multiple="false" name="priority" 
            classname="col-xs-4" apiEndpoint="/api/dependency/priorities" :value="priority" :onChange="onChange">
          </dynamic-select>
        </div>
      ...
      <faveo-box :title="lang('outgoing_email')">
        ...
        <status-switch slot="headerMenu" name="sending_status" :value="sending_status" :onChange="onChange"
          classname="pull-right" :bold="true">
        </status-switch>
        ...
        <div class="row">
          <!-- internal notes -->
          <text-field :label="lang('internal_notes')" :value="internal_notes" type="textarea" name="internal_notes" :onChange="onChange" classname="col-xs-12"/>

          <!-- make system default email -->
          <checkbox name="is_system_default" :value="is_system_default" :label="lang('make_system_default_mail')" :onChange="onChange" classname="col-xs-12"/>
        </div>
