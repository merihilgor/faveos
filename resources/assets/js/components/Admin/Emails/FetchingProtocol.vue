<template>
    <div class="box-body">
        ...
        <div :class="['protocol',{'protocol-settings': fetching_status == 1}]">
            <div class="row">
              <!--fetch-->
              <static-select :label="lang('fetching_protocol')"  :elements="fetching_protocols"
                             name="fetching_protocol" :value="fetching_protocol" classname="col-xs-3" :onChange="onChange"
                             :required="true">
              </static-select>

              <text-field :label="lang('host_name')" :value="fetching_host" name="fetching_host" classname="col-xs-3"
                          :onChange="onChange" :required="true">
              </text-field>

              <text-field id="fetching-port" v-if="shallPortBeVisible" :label="lang('port')" :value="fetching_port" name="fetching_port" classname="col-xs-3"
              ...
              <static-select id="fetching-encryption" v-if="shallEncryptionBeVisible" :label="lang('encryption')"  :elements="fetching_encryptions" name="fetching_encryption"
                             :value="fetching_encryption" classname="col-xs-3" :onChange="onChange" :required="true">
              </static-select>
            ...
            <div class="fetching-meta-actions">
              ...
              <checkbox id="delete-email" name="delete_email" :value="delete_email" :label="lang('delete_mail_after_fetch')"
                        :onChange="onChange" classname="col-xs-4" :hint="lang('delete_mail_notes')">
              </checkbox>
            </div>
