...
class DatabaseSeeder extends Seeder
{
    ...
    private function removeResolvedStatus()
    {
        if(!in_array(Config::get('app.env'), ["development", "testing"])) {
            ...
            if($resolved) {
                $ticketsWereInReslovedAnyTime = ActivityLog::where('field', 'status')
                                    ->where(function($qeury) use($resolved){
                                        return $qeury->where('initial_value', $resolved->id)
                                            ->orWhere('final_value', $resolved->id);
                                    })->count();
