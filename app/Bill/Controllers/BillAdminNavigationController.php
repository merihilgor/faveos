...
class BillAdminNavigationController
{
  use NavigationHelper;
    ...
    public function injectBillAdminNavigation(Collection &$coreNavigationContainer)
    {
        ...
        $navigationObject->setIconClass('fa fa-usd');

        $navigationObject->setHasChildren(true);

        $this->injectChildNavigation($navigationObject, 'options', 'fa fa-wrench', 'bill', 'bill');
        $this->injectChildNavigation($navigationObject, 'package', 'fa fa-suitcase', 'bill/package/inbox', 'bill/package/inbox');
        $this->injectChildNavigation($navigationObject, 'payment_gateway', 'fa fa-money', 'bill/payment-gateways', 'bill/payment-gateways');
