    ...
    <div class="box-body">
      ...
      <div class="row" v-if="!loading" id="invoice_filter">
        ...
        <date-time-field :label="lang('created_date')" :value="created_date" type="datetime" name="created_date"
            :onChange="onChange" range :required="false" format="YYYY-MM-DD HH:mm:ss" classname="col-xs-4" 
        ...
        <number-field :label="lang('amount_paid')" :value="amount_paid" name="amount_paid" classname="col-xs-2"
          :onChange="onChange" type="number">
        
        </number-field>

        <number-field :label="lang('payable_amount')" :value="payable_amount" name="payable_amount" classname="col-xs-2"
          :onChange="onChange" type="number">
      ...
      <div class="row" v-if="!loading">
        ...
        <static-select :label="lang('status')"  :elements="statuses" name="status"
          :value="status" classname="col-xs-4 invoice_select" :onChange="onChange" :required="false">
        
        </static-select>

        <dynamic-select name="payment_mode" classname="col-xs-4" :elements="modes" :multiple="true"
          :prePopulate="false" :label="lang('payment_mode')" :value="payment_mode" :onChange="onChange">
        
        </dynamic-select>

        <dynamic-select name="users" apiEndpoint="api/dependency/users?meta=true" classname="col-xs-4"  :multiple="true"
...
<style>
  #invoice_filter #number{
    background: transparent;
  }
  .invoice_select #static-select{
