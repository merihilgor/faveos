...
trait ApprovalThreadHandler
{
   ...
   * @return null
   */
  protected function createThread($category, $referer, string $action, $comment = "", string $actionPerformer = null)
  {
      switch ($category) {
      ...
      if(!$this->thread){
        $this->thread = !$this->thread ? new Thread : $this->thread;
        $this->thread->is_internal = 1;
        $this->thread->ticket_id = $this->ticket->id;
        $this->thread->body = '';
      }
      $this->thread->poster = 'support';
      $this->thread->thread_type = 'internal_activity_alert';
      $this->thread->body = $this->thread->body.$internalNoteBody;
  }
