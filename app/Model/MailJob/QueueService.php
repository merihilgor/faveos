...
class QueueService extends Model
{
    ...
    public function getAction(){
        $id = $this->attributes['id'];
        $status = $this->attributes['status'];
        $html = "<a href=".url('queue/'.$id.'/activate')." class='btn btn-primary btn-xs'><i class='fa fa-check-circle'>&nbsp;&nbsp;</i>".Lang::get('lang.activate')."</a>";
        if($status==1){
            $html = "<a href='#' class='btn btn-primary btn-xs' disabled><i class='fa fa-check-circle'>&nbsp;&nbsp;</i>".Lang::get('lang.activate')."</a>";
        }
        return $html;
    }
