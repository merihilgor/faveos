...
//    }
    ...
    {
        return $this->hasOne('App\Model\Common\ActivityLog', 'type_id')
                ->where('type', 'ticket')->where('is_updated', 1)->orderBy('updated_at', 'DESC');
    }
    ...
    private function departmentMatchesWithAssigned(?int $assignedId, int $deptId):?int
    {
        ...
        if((DepartmentAssignAgents::where("department_id", $deptId)->where("agent_id", $assignedId)->count() || (new TicketPolicy($assignedId))->globalAccess())){
            return $assignedId;
        }
