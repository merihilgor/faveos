...
import ProblemAssets from '../../../../../views/js/components/Problem/View/MiniComponents/ProblemAssets.vue';
...
describe('ProblemAssets',()=>{

	let wrapper;

	const updateWrapper = ()=>{

		wrapper = mount(ProblemAssets,{
	...
	it("`loading` value should be false if api returns errorresponse",(done)=>{
		...
		setTimeout(()=>{
			...
			expect(moxios.requests.mostRecent().url).toBe('/service-desk/api/problem-attach-assets/1')
	...
	it('updates `apiUrl` value when `onApply` method called',()=>{
		...
		expect(wrapper.vm.$data.apiUrl).toBe('/service-desk/api/asset-list?problem_id=1&asset_type_ids=1');
	})
	...
	function mockSubmitRequest(){
		...
		moxios.stubRequest('/service-desk/api/problem-attach-assets/1',{
