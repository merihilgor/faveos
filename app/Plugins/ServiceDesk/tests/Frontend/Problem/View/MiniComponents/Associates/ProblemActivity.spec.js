...
import ProblemActivity from '../../../../../../views/js/components/Problem/View/MiniComponents/Associates/ProblemActivity.vue';
...
describe('ProblemActivity',() => {
	...
	it('updates `paramsObj` value and calls `commonFilter` method when `orderBy` method called',()=>{
		...
		expect(wrapper.vm.paramsObj).toEqual({"sort-order": "asc", "page": 1})
	...
	it('makes an API call', (done) => {
		...
		setTimeout(()=>{
			...
			expect(moxios.requests.mostRecent().url).toBe('/service-desk/api/problem-log/1?page=1')
	...
	it("update `data` values when `getUpdateActivity` method called",(done)=>{

		wrapper.vm.getValues = jest.fn();

		wrapper.vm.getUpdateActivity();

		expect(wrapper.vm.activity_log).toEqual([]);
	...
	function stubRequest(status = 200,url = '/service-desk/api/problem-log/1?page=1'){
