...
import ReleaseAssets from '../../../../../../../views/js/components/Agent/Release/View/Mini/Child/ReleaseAssets.vue';
...
describe('ReleaseAssets',()=>{
	...
	it("`loading` value should be false if api returns errorresponse",(done)=>{
		...
		setTimeout(()=>{
			...
			expect(moxios.requests.mostRecent().url).toBe('/service-desk/api/release/attach-assets/1')
	...
	it('updates `apiUrl` value when `onApply` method called',()=>{
		...
		expect(wrapper.vm.$data.apiUrl).toBe('/service-desk/api/asset-list?release_id=1&asset_type_ids=1');
	})
	...
	function mockSubmitRequest(){
		...
		moxios.stubRequest('/service-desk/api/release/attach-assets/1',{
