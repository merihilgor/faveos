...
import ReleaseActivity from '../../../../../../views/js/components/Agent/Release/View/Mini/ReleaseActivity.vue';
...
describe('ReleaseActivity',() => {
	...
	it('makes an API call', (done) => {
		...
		setTimeout(()=>{
			...
			expect(moxios.requests.mostRecent().url).toBe('/service-desk/api/release-activitylog/1?page=1')
	...
	it('`checkDate` method returns undefined(formattedDate method returns empty value so i am assuming it retuns undefined) if it called with one',()=>{
		...
		expect(wrapper.vm.checkDate(1)).toEqual(false)
	})
	...
	function stubRequest(status = 200,url = '/service-desk/api/release-activitylog/1?page=1'){
