...
import ContractTypeCreateEdit from '../../../../views/js/components/Admin/ContractType/ContractTypeCreateEdit.vue';

import moxios from 'moxios';

import * as extraLogics from "helpers/extraLogics";

import * as validation from "../../../../views/js/validator/contractTypeSettings";

jest.mock('helpers/responseHandler')

describe('ContractTypeCreateEdit',() => {
	...
	const updateWrapper = () =>{
		...
		extraLogics.getIdFromUrl = () =>{return 1}
			...
			wrapper = mount(ContractTypeCreateEdit,{
	...
	beforeEach(() => {
		...
		moxios.stubRequest('/service-desk/api/contract-type/1',{
	...
	it('makes an API call', (done) => {
		...
		setTimeout(()=>{
			...
			expect(moxios.requests.mostRecent().url).toBe('/service-desk/api/contract-type/1')
	...
	it('updates `name` of the ContractType when onChange method is called with suitable parameters',()=>{
	...
	it('makes an AJAX call when onSubmit method is called',(done)=>{
		...
		setTimeout(()=>{
			...
			expect(moxios.requests.mostRecent().url).toBe('/service-desk/api/contract-type')
	...
	it('makes an loading as false when onSubmit method return error',(done)=>{
		...
		wrapper.setData({ loading : false, hasDataPopulated : true,contract_type_id : 1})
	...
	it("updates `title value when page is in edit`",()=>{
		...
		expect(wrapper.vm.$data.title).toEqual('edit_contract_type')
	});
	...
	it('isValid - should return false ', done => {
		...
		validation.validateContractTypeSettings = () =>{return {errors : [], isValid : false}}
	...
	it('isValid - should return true ', done => {
		...
		validation.validateContractTypeSettings = () =>{return {errors : [], isValid : true}}
	...
	function mockSubmitRequest(status = 200,url = '/service-desk/api/contract-type'){
	...
	function stubRequest(status = 200,url = '/service-desk/api/contract-type/1'){
