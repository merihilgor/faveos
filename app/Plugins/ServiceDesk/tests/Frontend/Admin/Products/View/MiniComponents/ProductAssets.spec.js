...
import ProductAssets from '../../../../../../views/js/components/Admin/Products/View/MiniComponents/ProductAssets.vue';
...
describe('ProductAssets',()=>{
	...
	const updateWrapper = ()=>{
		...
		wrapper = mount(ProductAssets,{

			 propsData : { productId : 1 },
	...
	it("`loading` value should be false if api returns errorresponse",(done)=>{
		...
		setTimeout(()=>{
			...
			expect(moxios.requests.mostRecent().url).toBe('/service-desk/api/product/attach-asset')
	...
	it('updates `apiUrl` value when `onApply` method called',()=>{
		...
		expect(wrapper.vm.$data.apiUrl).toBe('/service-desk/api/asset-list?asset_type_ids=1');
	})
	...
	function mockSubmitRequest(){
		...
		moxios.stubRequest('/service-desk/api/product/attach-asset',{
