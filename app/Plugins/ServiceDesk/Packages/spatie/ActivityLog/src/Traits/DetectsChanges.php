...
use App\Plugins\ServiceDesk\Packages\spatie\ActivityLog\src\Exceptions\CouldNotLogChanges;
    ...
    public static function logChanges(Model $model): array
    {
        ...
        foreach ($attributes as $attribute) {
            if (Str::contains($attribute, '.')) {
                $columns = explode('.', $attribute);
                $attribute = $columns[0];
                $subAttribute = $columns[1];
                $changes[$attribute] = $model->getAttribute($attribute);
                $changes[$attribute] = $changes[$attribute]->$subAttribute ?? $changes[$attribute];

            }

            $changes[$attribute] = array_key_exists($attribute, $changes) ? $changes[$attribute] : $model->getAttribute($attribute);
