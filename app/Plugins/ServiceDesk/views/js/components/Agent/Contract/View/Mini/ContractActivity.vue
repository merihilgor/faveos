<template>
	...
	<div class="row" id="contract_activity_div">
		...
		<div class="col-md-12">
			...
			<div class="dropdown" id="choose_order" v-if="!loading && activity_log.length > 0">
				...
				<button id="sort_btn" type="button" data-toggle="dropdown" class="btn btn-default dropdown-toggle">{{trans('created_time')}} - {{trans(sort_key)}}
				...
				<ul class="dropdown-menu" style="z-index: 9999;">
					...
					<li>
						<a href="javascript:;" @click="orderBy('asc')"><i class="fa fa-sort-amount-asc"></i> {{trans('asc')}}</a>
					</li>

					<li>
						<a href="javascript:;" @click="orderBy('desc')"><i class="fa fa-sort-amount-desc"></i> {{trans('desc')}}</a>
					</li>
				</ul>
			</div>
...
<script>
	...
	export default {
		...
		description : 'Contract activity page',
		...
		data(){
			...
			return {
				...
				apiUrl : '/service-desk/api/contract-log/'+this.contractId,
		...
		watch:{
			...
			"pagination.currentPage"(newValue,oldValue){
				...
				var elmnt = document.getElementById('contract_activity_div');
		...
		methods : {
			...
			getValues(params){
				...
				axios.get(this.apiUrl,{params}).then(res=>{
					...
					this.activity_log = res.data.data.contract_activity_logs.data;

					this.total = res.data.data.contract_activity_logs.total;

					this.perPage = res.data.data.contract_activity_logs.per_page;
