...
<script type="text/javascript">
	...
	export default {
		...
		description : 'Chnage asset Modal component',
		...
		methods:{
			...
			onSubmit(){
				...
				if(this.assetIds.length > 0){
					...
					axios.post('/service-desk/api/change/attach-assets/'+this.changeId,data,config).then(res => {

						this.loading = false

						window.eventHub.$emit('ChangeAssociatesAction','assets');
						
						window.eventHub.$emit('updateActivity');

						successHandler(res,'changes-view')

						window.eventHub.$emit('changeAssetsrefreshData');
