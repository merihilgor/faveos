<template>
	...
	<div class="row" id="release_activity_div">
...
<script>
	...
	export default {
		...
		description : 'Release activity page',
		...
		data(){
			...
			return {
				...
				apiUrl : '/service-desk/api/release-activitylog/'+this.releaseId,
		...
		watch:{
			...
			"pagination.currentPage"(newValue,oldValue){
				...
				var elmnt = document.getElementById('release_activity_div');
		...
		methods : {
			...
			getValues(params){
				...
				axios.get(this.apiUrl,{params}).then(res=>{
					...
					this.activity_log = res.data.data.release_activity_logs.data;

					this.total = res.data.data.release_activity_logs.total;

					this.perPage = res.data.data.release_activity_logs.per_page;
			...
			checkDate(x){
				...
				if(x) {
					...
					date1=this.formattedDate(this.activity_log[x-1].created_at);
				
					date2=this.formattedDate(this.activity_log[x].created_at);
				}
				
				return !x || (date1!=date2) ? true : false;
			},
