...
<script type="text/javascript">
	...
	export default {
      ...
      detachId:{type:String|Number , default : ''},

      compName:{type:String , default : ''},

      alertCompName : {type:String , default : 'product-view'},
      ...
      return {
		...
		onSubmit(){
			...
			let endPoint = this.compName === 'ProductAssets' ? '/service-desk/api/product/detach-asset/'+this.productId + '/' + this.detachId : '/service-desk/api/product/detach-vendor/'+this.productId+'/'+this.detachId

			axios.delete(endPoint).then(res=>{

        successHandler(res,'product-view');

        window.eventHub.$emit('updateProductData');  
