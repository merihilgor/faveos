<template>
	...
	<div class="row" id="problem_activity_div">
	...
	export default {
		...
		data(){
			...
			return {
				...
				apiUrl : '/service-desk/api/problem-log/'+this.problemId,
		...
		created(){
			...
			window.eventHub.$on('updateProblemLogs',this.getUpdateActivity)
		},
		...
		watch:{
			...
			"pagination.currentPage"(newValue,oldValue){
				...
				var elmnt = document.getElementById('problem_activity_div');
		...
		methods : {
			...
			getValues(params){
				...
				axios.get(this.apiUrl,{params}).then(res=>{
					...
					this.activity_log = res.data.data.problem_activity_logs.data;

					this.total = res.data.data.problem_activity_logs.total;

					this.perPage = res.data.data.problem_activity_logs.per_page;
