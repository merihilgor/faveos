<template>
	...
	<modal v-if="showModal" :showModal="showModal" :onClose="onClose" :containerStyle="containerStyle">
		...
		<div slot="title"><h4>{{lang('attach_assets')}}</h4></div>
...
<script type="text/javascript">
	...
	export default {
		...
		description : 'Problem Asset Modal component',
		...
		methods:{
			...
			onApply(){
				...
				let baseUrlForFilter = '/service-desk/api/asset-list?problem_id='+this.problemId+'&';
			...
			onSubmit(){
				...
				if(this.assetIds.length > 0){
					...
					axios.post('/service-desk/api/problem-attach-assets/'+this.problemId,data,config).then(res => {
...
<style type="text/css">
	...
	#static_select{ margin-left: 15px; margin-right: 15px;}

	.height-limit{ overflow-y: auto; max-height: 300px;overflow-x: hidden;}
</style>