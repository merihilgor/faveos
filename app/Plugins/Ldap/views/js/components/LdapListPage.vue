<template>
   <faveo-box :title="trans('ldap_list')">
      ...
      <span slot="headerMenu">
         ...
         <a class="btn btn-primary pull-right" :href="basePath() + '/ldap/settings/create'">
         	<span style="padding-right: 5px;" class="fa fa-plus" aria-hidden="true"></span>
         	{{trans('configure_new_ldap')}}
         </a>
            ...
            <div class="row">
               <div class="col-md-2"  v-tooltip="trans(list.schema)">
                  <div class="ldap-schema-img">
                     <img :src="list.image_url" :alt="list.schema">
                  </div>
               </div>
               ...
               <div class="col-md-2 description-block action-block float-right">
                  <a class="btn btn-primary btn-xs" :href="basePath() + '/ldap/settings/' + list.id + '/edit'"><i class="fa fa-edit" aria-hidden="true"  v-tooltip="trans('edit')"></i></a>
                  <button class="btn btn-danger btn-xs" @click="deleteItem(list.id)"><i class="fa fa-trash" aria-hidden="true"  v-tooltip="trans('delete')"></i></button>
               </div>
            </div>
         </div>
      </div>
