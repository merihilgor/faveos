 ...
 <div class="row" v-if="loading === true">
    <custom-loader :duration="loadingSpeed"></custom-loader>
      ...
      <modal v-if="deletePopup"  :classname="'modal-sm'" :containerStyle="containerStyle" :showModal="deletePopup" :onClose="onClose">
        ...
        <div slot="title" class="delete-search-base-modal-title">
          <h4 class="m-clear">{{lang('delete_search_base')}}</h4>
        </div>
        <!-- SLOT TITLE END -->
        <!-- SLOT FIELD  -->
          <div slot="fields">
            <div class="container">
              <div class="row">
                <div class="col-sm-12">
                  <h4 class="text-left m-clear">{{ trans('delete_confirmation_message') }}</h4>
                </div>
              </div>
            </div>
          </div>
      ...
      <!-- SEARCH BASE QUERY UI -->
        <faveo-box :title="lang('import_settings')">
            ...
            <div>
              ...
              <div  v-for="(user,index) in searchBaseArray" :key="index" class="clearfix ldap-import-query">
                ...
                <text-field  :id="'searchbase-' +index"  :label="lang('search_base')" :value="user.search_base" type="text" :name="'searchbase'+ index"
                        :onChange="onSearchBase" classname="col-xs-4" :required="true">
                </text-field>
                <!-- STATE BASIS USER END -->

                <!-- STATE BASIS USER -->
                <text-field  :id="'filter-' +index"  :label="lang('filter')" :value="user.filter" type="text" :name="'filter-'+ index"
                        :onChange="onFilter" classname="col-xs-6" :hint="lang('ldap_filter_hint')">
                </text-field>
                <!-- STATE BASIS USER END -->
                ...
                <static-select :id="'user-type-'+index" v-if="showRole" :label="lang('user_type')"   :elements="usersArray"
                     :name="index" :hideEmptySelect="true" :value="user.user_type" classname="col-xs-4" :onChange="onHandle"
                     :required="true">
                </static-select>
                <!-- USER TYPE END -->

                <!-- DEPARTMENTS -->
                <div v-if="user.user_type !=='user'  && showDepartment">
                  ...
                  classname="col-xs-6"  :value="user.departments" :onChange="onDepartmentChange">
                  </dynamic-select>
                </div>
                ...
                <!-- ORGANIZATIONS -->
                <div v-if="user.user_type==='user' &&  showOrganization">
                    <dynamic-select :key="'search-base-'+index+'-organization'" :id="'search-base-'+index+'-organization'" apiEndpoint="/api/dependency/organizations"   :label="lang('organizations')"  :multiple="true" :name="index" :prePopulate="false"
                      classname="col-xs-6"  :value="user.organizations" :onChange="onOrganizationChange">
                    </dynamic-select>
                </div>
          ...
          <!--SAVE BUTTONS  -->
          <div slot="actions">
            <button class="btn btn-primary update-btn" id="query" @click="saveandImport(searchBaseArray , false)" :disabled="loading"><span class="fa fa-refresh"></span>&nbsp;{{lang('save')}} </button>
            <button class="btn btn-primary update-btn" id="importquery" @click="saveandImport(searchBaseArray,true)" :disabled="loading"><span class="fa fa-refresh"></span>&nbsp;{{lang('save_and_import')}}</button>
            <button class="btn btn-primary update-btn" id="importquery" @click="addQuery" :disabled="loading"><span class="fa fa-plus"></span>&nbsp;{{lang('add_more')}}</button>
          </div>
