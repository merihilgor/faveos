...
class ApiLdapController extends BaseLdapController
{
    ...
    public function appendMetaSettings(&$data)
    {
        $ldapMetaSettings = Cache::rememberForever('ldap_meta_settings', function () {
            ...
            $hideDefaultLogin = (bool)CommonSettings::where('option_name', 'hide_default_login')->where('optional_field', 'ldap')->value('option_value');
            return ['hide_default_login'=> $hideDefaultLogin, 'directory_settings'=> $ldapMetaSettings];
        });
    ...
    public function getAdvancedSettings(int $ldapId)
    {
        ...
        FormField::getUserCustomFieldList()->map(function ($formField) use ($ldap, $defaultAdAttributeId) {
    ...
    public function getLdapSettingsList()
    {
        ...
        return successResponse('', ['ldap_list'=>$ldapList, 'hide_default_login'=> $hideDefaultLogin]);
    }
