...
class FacebookCronProcessor
{
    ...
    public function processFacebookMessages()
    {
        ...
        foreach ($messages as $message) {
            ...
            list('username' => $username,'email' => $email,'userId' => $userId,'body' => $body,'attachments' => $attachments, 'messageId' => $messageId, 'pageId' => $pageId, 'storagePath' => $storagePath) = $this->prepareTicketData($message);
    ...
    private function prepareAttachments($message)
    {
        ...
        if ($message['attachment_urls']) {
            ...
            foreach ($attachments as $attachment) {
                ...
                file_put_contents(storage_path('app' . DIRECTORY_SEPARATOR. $filename), $contents);
                $storagePath = storage_path('app' . DIRECTORY_SEPARATOR. $filename);

                //return uploadedFile instance in array format
                $pathArray = explode(DIRECTORY_SEPARATOR, $storagePath);
                $uploadedFile = new UploadedFile($storagePath, end($pathArray), null, 0, false);
                $uploads['uploads'][] = $uploadedFile;
                $uploads['storagePath'][] = $storagePath;
            }
        }
        return $uploads;
    }
    ...
    private function prepareTicketData($message)
    {
        ...
        $ticketColumns['attachments'] = $ticketColumns['storagePath'] = [];
        if ($attachmentsArray) {
            $ticketColumns['attachments'] = $attachmentsArray['uploads'];
            $ticketColumns['storagePath'] = $attachmentsArray['storagePath'];
        }
    ...
    private function reply($body, $ticketId, $attachments)
    {
        ...
        return $this->ticketController->saveReply(
            $ticketId, $body, requester($ticketId), true, $attachments, [], true, "client"
        );
    }
