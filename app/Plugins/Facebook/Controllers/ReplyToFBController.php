...
class ReplyToFBController extends Controller
{
        ...
        if ($body) {
            ...
            $message = [
                "attachment" => [
                    ...
                    "type" => ($inlineImage)
                        ? "image"
                        : (($attachment['type'] != 'application') ? $attachment['type'] : "file"),

                    "payload" => [
                        "url" => ($inlineImage)
                            ? $this->rawEncodeFileName($inlineImage)
                            : $this->constructAttachmentAndReturnUrl($attachment)
                    ]
                ]
            ];
        }
    ...
    private function prepareForReply($data, $userId)
    {
        ...
        $allowedExtensions = array_diff(config('filesystems.allowed_mime_types_public'), ['zip','rar','csv']);
        foreach ($attachments as $attachment) {
            if (!in_array(strtolower($attachment['extension']), $allowedExtensions)) {
