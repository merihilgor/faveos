<template>
	...
	<modal v-if="showModal" :showModal="showModal" :onClose="onClose" @close="showModal = false" 
		...
		<div slot="title">
			...
			<h4>{{lang('edit')}} {{this.data.name}}</h4>
		</div>
		...
		<div v-if="!loading" slot="fields">
			...
			<div class="row">
				...
				<div class="col-xs-12">
					...
					:onChange="onChange" classname="col-xs-6" :hint="lang('telephony_app_id_hint')"></text-field>

					<text-field :label="lang('api_token')" :value="token" type="text" name="token"
					:onChange="onChange" classname="col-xs-6" :hint="lang('telephony_app_token_hint')"></text-field>
				</div>
				<div class="col-xs-12">
					<text-field :label="lang('conversion_waiting_time')" :value="conversion_waiting_time" type="text" name="conversion_waiting_time"
					:onChange="onChange" classname="col-xs-6" :hint="lang('conversion_waiting_time_hint')"></text-field>

					<dynamic-select :label="lang('select_default_region')" :multiple="false"
					name="iso" :required="true" :prePopulate="true" classname="col-xs-6" apiEndpoint="/telephony/api/get-regions-list" :value="iso" 
				...
				<div class="col-xs-12">
					...
					classname="form-group col-xs-6" :hint="lang('telephony_log_missed_call_hint')" >
					</radio-button>
				</div>
			</div>
		</div> 
