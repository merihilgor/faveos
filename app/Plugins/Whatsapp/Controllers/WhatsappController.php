...
class WhatsappController extends Controller
{
    ...
    public function webhookProcess()
    {
        ...
        foreach ($WhatsAppMessageStore as $message) {
            if ($message['NumMedia'] == 1) {
                ...
                $this->generateTicket($message['Body'], $message['From'], $storagePath, $mimeType);
            } else {
    ...
    {
        file_put_contents(storage_path("app" . DIRECTORY_SEPARATOR . $filename), $content);
        return storage_path("app" . DIRECTORY_SEPARATOR . $filename);
    }
    ...
    private function generateTicket($body, $from, $storagePath = "", $mimeType = "")
    {
        ...
        if ($mimeType && $storagePath) {
            $producedAttachments = $this->makeAttachmentsFromPath($storagePath, $mimeType);
            $isAttachmentNotInline = $this->isThisAttachmentNotInline($mimeType);
            ($isAttachmentNotInline)
                ? array_push($attach, array($producedAttachments['uploaded_file']))
                : array_push($inline, array($producedAttachments['uploaded_file']));
        }
        $body = "<p>$body</p>";
        if ($producedAttachments && !$isAttachmentNotInline) {
            $body .= "<figure class='image'><img src='" . $producedAttachments['image_name'] . "'></figure>";
        }
    ...
    private function isThisMessageAReply($from)
    {
        ...
        if ($whatsAppMessageStore) {
            $checkReplyTime = $ticketController->checkReplyTime($whatsAppMessageStore->ticket_id);
     ...
     * @return array
     */
    private function makeAttachmentsFromPath($imagePath, $mimeType)
    {
        if (!$imagePath) {
            return [];
        }
        $imagePathArray = explode(DIRECTORY_SEPARATOR, $imagePath);
        $clientOriginalName = end($imagePathArray);
        $up = new UploadedFile($imagePath, $clientOriginalName, $mimeType, 0, false);
        return ["image_name" => $clientOriginalName, "uploaded_file" => $up];
    }
