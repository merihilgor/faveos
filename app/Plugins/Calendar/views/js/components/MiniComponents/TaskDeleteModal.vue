    ...
    <div>
        <modal v-if="showModal" :showModal="showModal" :onClose="onClose" :containerStyle="containerStyle">
            ...
            <div v-if="!loading" slot="fields">
                <h5 id="H5" :class="{margin: lang_locale == 'ar'}">
                    {{ (componentTitle === 'TaskList') ? lang('task-plugin-delete-confirmation-tasklists') : lang('task-plugin-delete-confirmation-projects')}}
                </h5>
            </div>
            ...
            <div slot="controls">
                <button type="button" @click = "onSubmit" class="btn btn-danger" :disabled="isDisabled"><i class="fa fa-trash" aria-hidden="true"></i> {{lang('delete')}}</button>
            </div>
