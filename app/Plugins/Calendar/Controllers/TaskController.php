...
class TaskController extends Controller
{
    ...
    {
        $taskById = Task::with(['taskList','taskList.project','assignedTo','ticket:id,ticket_number'])->where('id', $id)->first();
        return ($taskById)
    ...
    public function returnTasks(Request $request)
    {
        ...
        $query->when((bool)($request->projects), function ($q) use ($request) {
            return $q->whereHas('taskList.project', function ($qq) use ($request) {
                return $qq->whereIn('id', $request->projects);
            });
        });
        $query->when((bool)($request->task_lists), function ($q) use ($request) {
            return $q->whereHas('taskList', function ($qq) use ($request) {
                return $qq->whereIn('id', $request->task_lists);
            });
        });
        ...
        $tasks =  $query
            ->select('id', 'task_name', 'created_by', 'task_list_id', 'task_description', 'created_at', 'ticket_id', 'status', 'task_start_date', 'task_end_date')
    ...
    {
        $data = $request->except(['_token', 'alert_repeat', 'assignee', 'task_alert','task_list_id']);
        ...
        $data['task_list_id'] = $request->task_list_id ? $request->task_list_id['id'] : null;
        ...
        if ($task) {
            ...
            if ($request->assignee && count($request->assignee)) {
                foreach ($request->assignee as $assignee) {
                    $task->assignedTo()->create(["user_id" => $assignee]);
                }
    ...
    private function dispatcher($task, $event, $agents = [])
    {
    ...
    public function update(TaskRequest $request, Task $task)
    {
        ...
        $assigned = $cc = $newlyAssignedAgents = $existingAgents = [];
        $data = $this->formatRequestData($request);
        ...
        if ($task->is_private) {
            ...
            foreach ($request->assignee as $assignee) {
                array_push($assigned, $assignee);
                $task->assignedTo()->create(["user_id" => $assignee]);
            }

            $newlyAssignedAgents = array_diff($assigned, $agentsAssignedForTheTaskBeingUpdated);
            $existingAgents = array_intersect($assigned, $agentsAssignedForTheTaskBeingUpdated);
    ...
    private function baseTaskQuery()
    {
        ...
        return Task::with(['ticket:id,ticket_number','taskList:id,name'])->where(
    ...
    {
        $query = Task::where([['ticket_id','=', $request->ticket_id], ['status','!=','Closed']]);
        ...
        $tasks =  $query
            ->select('id', 'task_name', 'task_description', 'status', 'task_start_date', 'task_end_date')
            ->orderBy((($request->sort_field) ? : 'updated_at'), (($request->sort_order) ? : 'asc'))
            ->paginate((($request->limit) ? : '10'))
            ->toArray();
    ...
    {
        return Task::with(['ticket:id,ticket_number','taskList:id,name'])->whereHas('assignedTo', function ($q) {
