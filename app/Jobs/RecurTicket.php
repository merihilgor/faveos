...
class RecurTicket implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        $headers       = $this->getCCUserEmails($this->values['cc']);
        ...
        $this->ticketController()->create_user($user->email, $user->user_name, $subject, $body, "", "", "", $helptopic, $sla, $priority, $source, $headers, $dept, $assignto, $from_data, "", $status, $type, $attachment, $inline, $email_content, $company, 0,true,$locationId);
    }
    ...
    /**
     * gets email Ids of the cc users
     * @param  array $ccIds
     * @return array
     */
    private function getCCUserEmails($ccIds = null) : array
    {
      if($ccIds && is_array($ccIds)){
        return User::whereIn('id',$ccIds)->pluck('email')->toArray();
      }
      return [];
    }
}
