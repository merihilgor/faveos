...
class User extends BaseModel implements AuthenticatableContract, CanResetPasswordContract, JWTSubject
{
    ...
    protected $fillable = ['user_name', 'email', 'password', 'active', 'first_name', 'last_name', 'ext', 'mobile', 'profile_pic',
        ...
        'directory_listing', 'vacation_mode', 'role', 'internal_note', 'country_code', 'not_accept_ticket', 'is_delete', 'mobile_verify', 'email_verify', 'location', 'department', 'import_identifier','google2fa_secret','is_2fa_enabled','google2fa_activation_date', 'last_login_at', 'delete_account_requested', 'processing_account_disabling'];
    ...
    public function getProfilePicAttribute($value)
    {
        ...
        if ($value != ""){
         ...
         $profilePic = url('uploads' . '/' . 'profilepic' . '/' . '' . urlencode($value));
        }
        elseif ($this->attributes['email'] ) {

         $profilePic = \Gravatar::src($this->attributes['email']);
        }

        else{
            
         $profilePic = assetLink('image','contacthead');
        }
...
//    }
    ...
    {
        return $this->hasOne('App\Model\helpdesk\Agent\Permission', 'user_id');
    }
    public function sdPermission()
    {
        return $this->hasOne('App\Plugins\ServiceDesk\Model\Permission\AgentPermission', 'user_id');
    }
    public function save(array $options = array())
    {
    ...
    */
    ...
    {
        if(!$value){
            return Timezone::where('name', System::value('time_zone'))->value('id');
        }
        return $value;
...
//
    ...
    private function validateUser(User $user)
    {
        ...
        self::validateForAgentLimit($user);
    }
     ...
     * @throws AgentLimitExceededException
     */
    public static function validateForAgentLimit(User $user)
    {
        if(!Config::get("auth.agent_limit")){
            return true;
        }
        ...
        if($alreadyExistingAgentCount > Config::get("auth.agent_limit")){
