...
class DailyReport extends BaseReportController
{
     ...
     * @return mixed
     */
    private function getFormattedGeneralAgentReport(&$baseQuery, $reportTitle, $columnForDateTime = "duedate", $dateText = "due_at")
    {
        // selecting duedate 2 time so that query in TicketListController stays disrupted
        $result = $baseQuery->addSelect(["tickets.id", "ticket_number", "duedate", "$columnForDateTime as date_time"])
        ...
        $result->getCollection()->map(function ($element) use ($dateText, &$report) {
    ...
    private function getFormattedAgentAnalysisElement($object, $baseFilter)
    {
        ...
        $reportElement->title = $object->assigned ? $object->assigned->full_name : "";
