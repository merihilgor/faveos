...
class BaseTableExportTest extends DBTestCase
{
    ...
    public function test_shallCreateNewSheet_whenCurrentPageIsFirstPage_shouldReturnTrue()
    {
        $classObject = $this->getClassObject();

        $this->setPrivateProperty($classObject, 'page', 1);

        $methodResponse = $this->getPrivateMethod($classObject, 'shallCreateNewSheet');

        $this->assertTrue($methodResponse);
    }

    public function test_shallCreateNewSheet_whenItExceedsAllowedUpperLimit_shouldReturnTrueAndIncrementFileName()
    {
        $classObject = $this->getClassObject();

        // 100th page will be having 100000 limit, which is more than given default limit
        $this->setPrivateProperty($classObject, 'limit', 10000);
        $this->setPrivateProperty($classObject, 'page', 2);
        $this->setPrivateProperty($classObject, 'fileName', 'a');

        $methodResponse = $this->getPrivateMethod($classObject, 'shallCreateNewSheet');

        $newFileName = $this->getPrivateProperty($classObject, 'fileName');

        $this->assertTrue($methodResponse);
        $this->assertEquals('b', $newFileName);
    }

    public function test_shallCreateNewSheet_whenItDoesNotExceedUpperLimitButFilePathIsInvalid_shouldReturnTrue()
    {
        $classObject = $this->getClassObject();
    ...
    public function test_shallCreateNewSheet_whenItIsLowerThanAllowedUpperLimitAndPageIsNotOne_shouldReturnTrue()
    {
        $classObject = $this->getClassObject();

        $this->setPrivateProperty($classObject, 'page', 2);

        $this->setPrivateProperty($classObject, 'reportFilePath', base_path());
        $this->setPrivateProperty($classObject, 'report', (object)['ext'=>'']);
        $this->setPrivateProperty($classObject, 'fileName', '');
