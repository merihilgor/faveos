<template>
  <div id="custom-report-column">
    <modal :showModal="true" :onClose="closeView">

      <div slot="title">
        <p>{{lang('custom_column')}}</p>
        <alert componentName="custom-report-column"/>
        <custom-loader v-if="loading"></custom-loader>
      </div>

      <div slot="fields" id="link-container" class="custom-column-modal-body" refs="modalBodyRef">

        <div id="short-code-container">
          <div class="panel panel-default">
            <div class="panel-heading">{{lang('list_of_available_shortcodes')}}</div>
        ...
        <text-field :label="lang('column_name')" :value="name" name="name" :onChange="onChange" classname="col-xs-12" :hint="lang('report_custom_column_name_hint')">
        </text-field>

        <text-field :label="lang('equation')" :value="equation" name="equation" :onChange="onChange" classname="col-xs-12" type="textarea" :hint="lang('report_custom_column_equation_hint')">
        </text-field>

        <checkbox name="isTimestamp" :value="isTimestamp" :label="lang('is_timestamp')"
                  :onChange="onChange" classname="col-xs-12">
        </checkbox>


        <div v-show="isTimestamp">
          <div class="col-xs-6">
          ...
          <text-field name="timestamp_format_example" :label="lang('preview')" :value="getTimestampExample()" classname="col-xs-6" :disabled="true" :onChange="() => {}">
          </text-field>
        </div>
      ...
      <div slot="controls">
        ...
        <button type="button" id="custom-column-submit" @click="onSubmit" :disabled="loading" class="btn btn-primary update-btn">
          <span class="fa fa-refresh"></span>&nbsp;{{lang('submit')}}
        </button>
...
<style scoped>
  ...
  #report-shortcode-list {
    margin-left: -12px;
    margin-right: -12px;
  }

  #short-code-container .margin-10{
    margin-top: 5px;
    margin-bottom: 0px;
  }

  #short-code-container .box-body{
    padding-top: 5px;
    padding-bottom: 0px;
  }
