...
class TicketRecurController extends Controller
{
    ...
    */
    ...
    public function getRecurTicket($recurId)
    {
        ...
        $formattedRecurTicketContent['recur'] = $recur;

        return successResponse('', $formattedRecurTicketContent);
    }
    ...
    private function getDepartment($value)
    {
        ...
        return $dependencyDetails->getDependencyDetails('department_id', $value, true);
    }

    private function getHelpTopic($value)
    {
        $dependencyDetails = new DependencyDetails();
        $dependencyDetails->setOutputAsObject(true);
        return $dependencyDetails->getDependencyDetails('help_topic_id', $value, true);
    }
    ...
    private function formatRecurTicket($key, $value, DependencyDetails $dependencyDetails)
    {
        ...
        try {
            switch ($key) {
                ...
                case 'requester':
                    $dependencyDetails->setOutputAsObject(true);
                    return[$mapping[$key] => $dependencyDetails->getDependencyDetails($key, $value, in_array($key, $metaDependencies))];
    ...
    {
        $this->recurPermission = (new TicketPolicy)->accessRecurTicket();
