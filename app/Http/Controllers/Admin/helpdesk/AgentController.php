...
class AgentController extends Controller {
    ...
    */
    ...
    {
        $agent->permision()->delete();
        if (array_key_exists('permission_ids', $user)) {
            if (!empty($user['permission_ids'])) {
                $permissions= [];
                foreach ($user['permission_ids'] as $key => $value) {
                    $permissions[$value] = "1";
                }
                $agent->permision()->create(['user_id' => $agent->id, 'permision' => $permissions]);
            }
        }
    ...
    */
    public function editAgent($agentId)
    {
        $agent = User::where('role', '<>', 'user')
                       ->where('id', $agentId)
                       ->with(['timezone','permision','type.type:id,name','departments:department.id,department.name','teams:teams.id,teams.name'])
                       ->select('id', 'first_name', 'last_name', 'user_name', 'location', 'role', 'email', 'ext', 
                        'country_code', 'mobile', 'phone_number', 'active', 'agent_tzone', 'agent_sign' );
    ...
    */
    private function extraFormatAgent(&$member)
    {
        ...
        unset($member['permision'], $member['full_name']);
    }
