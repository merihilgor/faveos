    ...
    public function getStatusesTable()
    {
        ...
        return \DataTables::of($status)
            ...
            ->addColumn('action', function($status){
                $action = '<a href="'.route('status.edit',$status->id).'"><button class="btn btn-primary btn-xs"> <i class="fa fa-edit"> &nbsp;</i> '.Lang::get('lang.edit').'</button></a>&nbsp;&nbsp';
                if ($status->default == 1) {
                    $action .= '<a class="btn btn-primary btn-xs" disabled> <i class="fa fa-trash"> &nbsp;'.Lang::get('lang.delete').'</i></a>';
                } else {
                    $action .= '<a  class="btn btn-primary btn-xs" onClick="showConfirmModer('.$status->id.')"> <i class="fa fa-trash"> &nbsp;'.Lang::get('lang.delete').'</i></a>';
                }
                return $action;
            })
