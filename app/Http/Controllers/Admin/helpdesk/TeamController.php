...
class TeamController extends Controller {
    ...
    public function teamShow($id)
    {
        try {
            ...
            return view('themes.default1.admin.helpdesk.agent.teams.show', compact('teams', 'team_members', 'team_lead_name', 'html', 'ticket_policy'));
        } catch (Exception $e) {
            return redirect()->back()->with('fails', $e->getMessage());
        }
    }
    ...
    public function getTeamTable()
    {
        ...
        ->editColumn('status', function($teams){
            if($teams->status == 0) {
                return '<span class="btn btn-xs btn-default" style="color:red;pointer-events:none;">'.Lang::get('lang.inactive').'</span>';
            }
            return '<span class="btn btn-xs btn-default"  style="color:green;pointer-events:none;">'.Lang::get('lang.active').'</span>';
        })
        ...
        ->addColumn('action', function($teams){
            ...
            if ($teams->status == 1) {
                $show = '<a href="'.route('teams.profile.show', $teams->t_id).'" class="btn btn-primary btn-xs " ><i class="fa fa-eye" style="color:white;">&nbsp;&nbsp;</i>'.Lang::get('lang.view').'</a> ';
                    ...
                    $url = url('delete/teampopup/'.$teams->t_id);
                    $confirmation = deletePopUp($teams->t_id, $url, "Delete",'btn btn-primary btn-xs');
            
            return '<a href="'.route('teams.edit', $teams->t_id).'" class="btn btn-xs btn-primary"><i class="fa fa-edit" style="color:white;">&nbsp;</i> '.Lang::get('lang.edit').'</a>&nbsp;&nbsp;'.$show.'&nbsp;'.$confirmation
            .\Form::close();
        })
