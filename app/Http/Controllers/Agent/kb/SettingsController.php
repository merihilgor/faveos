...
class SettingsController extends Controller
{
    ...
    public function postSettings(SettingsRequests $request)
    {
        try {
            ...
            Settings::where('id', 1)->update(['pagination' => $request->pagination, 'status' => $request->status]);
    ...
    public function publish($commentId)
    {
        ...
        $comment->status = 1;
        if ($comment->save()) {
            return redirect('comment')->with('success', $comment->name . '-' . Lang::get('lang.comment_approaved_successfully'));
        } else {
    ...
    public function unapprove($commentId)
    {
        ...
        $comment->status = 0;
        if ($comment->save()) {
            return redirect('comment')->with('success', $comment->name . '-' . Lang::get('lang.comment_unapproaved_successfully'));
        } else {
            return redirect('comment')->with('fails', Lang::get('lang.can_not_process'));
        }
    }
