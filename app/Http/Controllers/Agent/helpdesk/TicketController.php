...
class TicketController extends Controller {
    ...
    public function post_newbatchticket(AgentPanelTicketRequest $request, CountryCode $code, $api = true){
        set_time_limit(-1);
        try{
            $data = (array)$request->except(['requester']);
            $file = $request->file('requester');
            $file_name  = \Storage::put('', $file[0]);
            ...
            foreach ($attachmentKeys as $key) {
              ...
              if($key != 'requester'){
                ...
                foreach ($data[$key] as $index => $attachment) {
                    ...
                    // filename
                    $data[$key][$index] = \Storage::putFileAs('batch', $attachment, $filename);
                }
              }
            }
            ...
            BatchTicketCreate::dispatch($data, $file_name);

            // save all custom attachments somewhere and its path in the value
            // of the field. While processing, re fetch those images and pass to ticket controller
    ...
    public function post_newticket(AgentPanelTicketRequest $request, CountryCode $code, $api = true) {
        try {
            $email = null;
            $username = null;
            $mobile_number = null;
            ...
            $locationId = $request->location_id ? $request->location_id : null;
    ...
    public function getModelValue($option, $value) {
        switch ($option) {
            case "type":
                return Tickettype::whereId($value)->select('id', 'name as optionvalue')->first();
            case "department":
                return Department::whereId($value)->select('id', 'name as optionvalue')->first();
            case "priority":
                return Ticket_Priority::where('priority_id', $value)->select('priority_id as id', 'priority as optionvalue')->first();
            case "cab":
                if (is_numeric($value)) {
                    return User::whereId($value)->select('id', 'user_name as name', 'first_name', 'last_name')->first();
                }
                return $value;
        }
    }
    ...
    public function createAttachments($attach, $key, $attachment, $thread, $drive, $poster = 'ATTACHMENT') {
        ...
        if (is_object($attachment)) {
            ...
            $storage->saveObjectAttachments($thread->id, $attachment, false, $poster);
    ...
    public function saveReplyTicket($ticket_id, $system_reply, $user = "", $internalNote=false) {
        ...
        // variable $due to store old duedate of ticket
// duedate has been handled in observer now
//        $due = $ticket->duedate;
...
//        if ($user && $user->role !== 'user' && $this->isFirstResponse($ticket_id)) {
//            $ticket = $this->setResponse($ticket, $due);
//        }
        $ticket->save();
    ...
    public function changeDuedate(Request $request) {
        ...
        if (!$this->ticket_policy->changeDuedate()) {
            return errorResponse(Lang::get('lang.permission_denied'));
        }
        ...
        $assignee = Auth::user()->first_name . ' ' . Auth::user()->last_name;
        $thread = new Ticket_Thread();
        $thread->ticket_id = $ticket->id;
        $thread->user_id = Auth::user()->id;
        $thread->is_internal = 1;
        $thread->body = 'This Ticket duedate changed by ' . $assignee;
        $thread->save();
        try{
    ...
    /**
     * NOTE : It has to be depreciated after new inbox gets merged
     * @param type $id
     * @param \Illuminate\Http\Request $request
     * @param type $api
     * @return type
     */
    public function ticketChangeDuedate($id, Request $request, $api = false) {
        ...
        $selected_date = $request->select_tkt_due_date_change;
        if ($selected_date == 'today') {
            $current_time = Carbon::today()->tz(timezone())->endofDay()->tz('UTC');
        } elseif ($selected_date == 'tomorrow') {
            $current_time = Carbon::parse('+1 days')->tz(timezone())->endofDay()->tz('UTC');
        } else {
            $timestamp = $request->menual_due_date;
            $current_time = Carbon::createFromFormat('Y-m-d H:i:s', $timestamp, System::First()->value('time_zone'));
            $current_time->setTimezone('UTC');
        }
        ...
        return redirect()->back()->with('success', Lang::get('lang.ticket_duedate_successfully_changed'));
    }
     ...
     * @return type bool
     */
    public function create_user($emailadd, $username, $subject, $body, $phone, $phonecode, $mobile_number, $helptopic, $sla, $priority, $source, $headers, $dept, $assignto, $form_data, $auto_response, $status, $type, $attachment = [], $inline = [], $email_content = [], $org = "",$domainId='', $loggedByAgent = false, $locationId = null) {
        $email;
        $username;
        $unique = $emailadd;
        ...
        $ticket_number = $this->check_ticket($user_id, $subject, $body, $helptopic, $sla, $priority, $source, $headers, $dept, $assignto, $form_data, $status, $type, $attachment, $inline, $email_content, $org,$domainId, $locationId);
     ...
     * @return type string
     */
    public function check_ticket($user_id, $subject, $body, $helptopic, $sla, $priority, $source, $headers, $dept, $assignto, $form_data, $status, $type, $attachment, $inline = [], $email_content = [], $company = "",$domainId="",$locationId) {
        ...
        if (!$createNewTicketResponse && $ticket) {
            ...
            //$this->updateTicketFromFetchedMail($ticket, $user);
             ...
             */
            \Event::dispatch('before_ticket_creation_event', [[$form_data, $user_id, $source, $dept]]);
            $ticket_number = $this->createTicket($user_id, $subject, $body, $helptopic, $sla, $priority, $source, $headers, $dept, $assignto, $form_data, $status, $type, $attachment, $inline, $email_content, $company,$domainId,'',$locationId);
    ...
    //Can be removed as the control goes to open() method and last update happens there
    /**
     * updates ticket if its purpose of status is not open
     * @param object $ticket   ticket which has to be updated
     * @param object $user     user by who triggered the action on the ticket
     * @return boolean         true for success, false for failure
     */
    public function updateTicketFromFetchedMail($ticket, $user){
        if ($ticket->statuses->purpose_of_status > 1) {
            $ticket->status = (($ticket->statuses->purpose_of_status == 2 || $ticket->statuses->purpose_of_status == 1) && $user->role == 'user')? \App\Helper\Finder::getTicketDefaultStatus() : $ticket->status;
            $ticket->closed = 0;
            $ticket->closed_at = date('Y-m-d H:i:s');
            $ticket->reopened = 1;
            $ticket->reopened_at = date('Y-m-d H:i:s');
            $ticket->save();
            $username = $user->role == 'user' ? $user->user_name : $user->first_name . ' ' . $user->last_name;
            $data = [
    ...
    public function getPriority($priority, $sla) {
        if ($sla) {
    ...
    public function createTicket($user_id, $subject, $body, $helptopic, $sla, $priority, $source, $headers, $dept, $assignto, $form_data, $status, $type, $attachment, $inline = [], $email_content = [], $company = "",$domainId="", $fork = false,$locationId = null) {
    ...
    public function customFormCreateold($form_data, $id) {
        if (is_array($form_data) && count($form_data) > 0) {
            foreach ($form_data as $key => $data) {
                if (is_array($data)) {
    ...
    public function assign(Request $request, $api=false)
    {
        ...
        if (Auth::user()->role == "agent") {
            ...
            if ($api && (!$ticket_policy->reAssigningTickets() || !$ticket_policy->assign())) {
                $message = ['error' => Lang::get('lang.permission_denied')];
                return response()->json(compact('message'), 403);
            }

            //when user has none of assign or reassign permission
            if(!$ticket_policy->reAssigningTickets() && !$ticket_policy->assign()){
            ...
            } else if ($ticket_policy->assign() && !$ticket_policy->reAssigningTickets()) {
     ...
     * @return type json
     */
    public function autoSearchticket() {

        $term = Input::get('q');
        $result = Ticket_Thread::where(function($q) use ($term) {
                    $q->where('title', 'LIKE', '%' . $term . '%');
                })->orWhereHas('ticket', function($q) use($term) {
                    $q->where('ticket_number', 'LIKE', '%' . $term . '%');
                })->groupBy('ticket_id')->pluck('title', 'ticket_id');
        ...
        if ($data == null) {
            ...
            if (($data->is_delete == 1)) {
                return '<div id="alert11" class="alert alert-warning alert-dismissable">'
                        . '<button id="dismiss11" type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>'
                        . '<i class="icon fa fa-ban"></i>'
                        . Lang::get('lang.the_email_belongs_to_ban_or_deactivated_account')
                        . '</div>'
                        . '</div>';
            }
        }
    ...
    public function useradd() {
        ...
        if ($user_search || $checkEmailSettings > 0) {
            return '<div id="alert11" class="alert alert-warning alert-dismissable" ><button id="dismiss11" type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button><h4><i class="icon fa fa-alert"></i>Alert!</h4><div id="message-success1">' . Lang::get('lang.user-exists') . '</div></div>';
        } else {
            $company = $this->company();
            $user = new User();
            ...
            return '<div id="alert11" class="alert alert-dismissable" style="color:#60B23C;background-color:#F2F2F2;"><button id="dismiss11" type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button><h4><i class="icon fa fa-user"></i>'. Lang::get('lang.cc_added_successfully'). '</h4><div id="message-success1"></div></div>';
        }
    }
        ...
        if (Input::has('select_all')) {
            ...
            foreach ($selectall as $delete) {
                ...
                if ($value == 'Delete') {
                    ...
                    //$notification = Notification::select('id')->where('modelid', '=', $ticket->id)->get();
                    //$notification = Notification::select('id')->where('modelid', '=', $ticket->id);
                    //$notification->delete();
     ...
     * @deprecated
     */
    private function sendToJobs($user_id, $mode, $data){
        switch($mode){
            case "email":
                $user = User::where('id', $user_id)->first();
                $templateVariables = array_merge($data, ['receiver_name' => $user->full_name]);
                    $job = (new \App\Jobs\Notifications("email-notification", $templateVariables, null, 'rating-confirmation', [], $user->email));
                    $this->dispatch($job);
            break;

            case "in-app-notify":
                $notify_data =  [
                    'message' => 'User has submitted a rating for '.$data['ticket_number'],
                    'to' => implode(',', $user_id),
                    'by' => '',
                    'table' => "tickets",
                    'row_id' => $data['row_id'],
                ...
                $job = (new \App\Jobs\Notifications("in-app-notification", $notify_data, 'rating-confirmation'));
                $this->dispatch($job);
            break;
        }
    ...
    public function mergeTickets($id) {
        ...
        foreach ($t_id as $value) {//to create new thread of the tickets to be merged with parent
            ...
            if ($thread) {
                ...
                //updating the thread
                Ticket_Thread::where('ticket_id', '=', $value)->update(['ticket_id' => $p_id]);
 ...
 */
    ...
    private function performStatusChangeAction($ticketIds, $userModel, $statusModel)
    {
        foreach ($ticketIds as $key => $ticketId) {
            ...
            $oldStatusModel = \App\Helper\Finder::status($ticket->status);
            ...
            if ($changed) {
                ...
                // NOTE FROM AVINASH : As per Giorgio's requirement, we've implemented status change last updated
                // time but keeping in mind that we have to implement activity_logs. This method and controller can
                // be used to update activity logs for the entire system

                // updating activity log
                (new ActivityLogController('updated','user', $userModel->id, $ticketId))
                    ->ticketActivityLog('status',$oldStatusModel->id, $statusModel->id);
    ...
    private function checkUserHasPermissionToChangeStatus($statusModel, $userModel)
    {
        ...
        switch ($statusPurpose) {
            case 'closed':
                return $this->ticket_policy->close();
            case 'deleted':
                return $this->ticket_policy->delete();
            case 'unapproved':
