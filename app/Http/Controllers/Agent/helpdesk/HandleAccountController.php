...
class HandleAccountController extends Controller
{
     ...
     * @return null
     */
    protected function handleOwnedTicketsForDeactivation(string $action='deactivated')
    {
        if($this->request['action_on_owned_tickets'] == 'change_owner'){

            return $this->changeOwnerOfUserTickets($action);
        }
     ...
     * @return void
     */
    protected function changeOwnerOfUserTickets($action):void
    {
        $openStatusIds = getStatusArray('open');
    ...
    {
        $newAssignee = User::find($newUserId);
        $this->user->ticketsAssigned()->chunkById(10, function($tickets) use($newUserId, $newAssignee){
            foreach ($tickets as $ticket) {
                $this->addTicketThreadLog($ticket, 'deactivated', 'assignee');
                $ticket->assigned_to = $newUserId;
                $ticket->save();
                $this->updateAssignmentLog($ticket, $ticket->assigned_to == $newUserId, $newAssignee);
            }
        });
    }
    ...
    protected function addTicketThreadLog(Tickets $ticket, string $action, string $role):void
    {
        $ticket->thread()->create([
            'body' => trans('lang.account_deactivate_delete_ticket_thread', [
    ...
    /**
     * Method to log ticket assignee update details on the ticket as internal note
     * @param  Tickets  $ticket           ticket to add log
     * @param  bool     $assigneeChanged  tells wether the assignee got updated or not
     * @param  User     $newUser          new assignee user, it can be null too(in case of surrender)
     * @return void
     */
    private function updateAssignmentLog(Tickets $ticket, bool $assigneeChanged, User $newUser=null):void
    {
        if($assigneeChanged) {
            $body = trans('lang.this-ticket-has-been-unassigned-by', [
                'profileURL' => url('/agent/'.$this->actor->id),
                'actor' => $this->actor->full_name,
            ]);
            if($newUser) {
                $body = trans('lang.this-ticket-has-been-assigned-to',[
                    'profileURL' => url('/agent/'.$newUser->id),
                    'assignee' => $newUser->full_name
                ]);
            }
            $ticket->thread()->create(['body' => $body, 'is_internal' => 1]);
        }
    }
