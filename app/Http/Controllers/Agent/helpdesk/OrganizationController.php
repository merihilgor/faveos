   ...
   /**
    ...
    public function orgListData(Request $request)
    {
        try {
            ...
            $baseQuery = Organization::select('id', 'name', 'website', 'phone','created_at','updated_at')->orderBy($sortBy, $orderBy);
    ...
    public function create() {
        try {
              if (!$this->ticket_policy->accessOrganizationProfile()){
 ...
 */
     ...
     public function createOrgApi(OrganisationCreateRequest $request)
     {
         ...
         try {
             ...
             if ($request->file('organisation_logo')) {
                ...
                $fileName = $this->imageStore( $request->file('organisation_logo'));

                $org = Organization::where('id', $org->id)->first();
                $org->logo =  '/uploads/company/'.$fileName;
                $org->save();
            }
    ...
    try {
            ...
            $org = Organization::whereId($id)->select('id','name as organisation_name','phone','website','address','head','internal_notes as description','domain','logo')->first();
            ...
            if ($org->logo) {
                ...
                $logo = [["file"=>'data:image/' . $type . ';base64,' . base64_encode($image), "filename"=>basename($path)]];
            }

            $orgInfo = (['organisation_domain_name'=>$orgDomains, 'organisation_logo'=>$logo,'organisation_department' => $orgDept]);
            
            $response = array_merge( $org->toArray(),$orgInfo,$customFields);
...
/**
    ...
    public function getOrgUserList(int $orgId, Request $request)
    {
        try {
            ...
            $baseQuery = User::whereIn('id', $userId)->select('id', 'first_name', 'last_name', 'user_name', 'phone_number', 'email', 'active', 'email_verify', 'mobile_verify', 'is_delete','updated_at', 'role')->orderBy($sortBy, $orderBy);
    ...
    public function organizationViewApi($orgId)
    {
        try {
            ...
            $organizationInfo->logo = $organizationInfo->logo ? url('/').$organizationInfo->logo : NULL;
