...
class TicketTimelineController extends TicketsCategoryController
{
     ...
     * @throws Exception
     */
    public function getTroubleTicket($ticketId)
    {
        //query for allowed ticket by logged in user
        $allowedTicketQuery = $this->allTicketsQuery();
        ...
        $formattedTicket['threads'] = $threadData['data'];

        foreach ($formattedTicket['threads'] as &$thread) {
            unset($thread['attach']);
        }

        $formattedTicket['thread_count']= TicketThread::where('ticket_id', $ticketId)->count();

        unset($formattedTicket['creator_id'], $formattedTicket['source'], $formattedTicket['isanswered'], $formattedTicket['status']['id'], $formattedTicket['status']['href'], $formattedTicket['updated_at'], $formattedTicket['organizations'], $formattedTicket['last_replier'], $formattedTicket['collaborators'], $formattedTicket['assignee'], $formattedTicket['type'], $formattedTicket['task_count'], $formattedTicket['is_overdue'], $formattedTicket['due_today'], $formattedTicket['collaborator_count'], $formattedTicket['attachment_count'], $formattedTicket['poster'], $formattedTicket['encrypted_id'], $formattedTicket['priority']['priority_color'], $formattedTicket['priority']['href'], $formattedTicket['departments'], $formattedTicket['sla_plan'], $formattedTicket['labels'], $formattedTicket['tags'], $formattedTicket['user'], $formattedTicket['creator'], $formattedTicket['helptopic']['href'], $formattedTicket['helptopic']['form_identifier'], $formattedTicket['location'], $formattedTicket['collaborator_count_relation']);

        foreach ($formattedTicket['custom_field_values'] as &$extraField) {
            if($extraField['label'] == 'TicketID'){
                unset($extraField['id'], $extraField['value'], $extraField['field_type']);
            } else {
                unset($extraField['id'], $extraField['href'], $extraField['field_type']);
            }
        }
        ...
        unset($formattedTicket['associated_problem']);
    ...
    private function getForeignKeyDataForTicket($ticketQuery)
    {
        $ticketQuery = $ticketQuery->with(['statuses:id,name', 'types:id,name', 'priority:priority_id,priority as name,priority_color', 'departments:id,name',
            ...
            'creator:id,first_name,last_name,user_name,email,profile_pic',
    ...
    private function formatTicket(&$ticket)
    {
        ...
        unset($ticket->user_id, $ticket->team_id, $ticket->dept_id, $ticket->priority_id, $ticket->sla,
            $ticket->type, $ticket->help_topic_id, $ticket->assigned_to, $ticket->firstThread, $ticket->lastThread,
            $ticket->sources, $ticket->location_id, $ticket->assigned, $ticket->assignedTeam);
     ...
     */
    public function ticketDetailsInEditMode($ticketId)
    {
        //query for allowed ticket by logged in user
        $allowedTicketQuery = $this->allTicketsQuery();
    ...
    private function appendHelptopicDepartmentChildFields(Tickets &$ticket)
    {
        ...
        $ticket->help_topic = $dependencyDetails->getDependencyDetails('help_topic_id', $ticket->help_topic_id, true);
        $ticket->department = $dependencyDetails->getDependencyDetails('department_id', $ticket->dept_id, true);
    }
    ...
    public function ticketConversationForAgent($ticketId, TicketDetailsRequest $request)
    {
        ...
        $threadData->getCollection()->transform(function($element){
            ...
            unset($element->user_id, $element->is_internal, $element->poster);
            return $element;
        });
    ...
    private function getThreadTypeIconDetails(TicketThread $thread)
    {
        if($thread->poster == 'client'){
            return (object)['icon_class'=> 'fa fa-comment bg-aqua', 'thread_type_text' => Lang::get('lang.posted_by_client')];
        }

        if($thread->thread_type == 'note' && $thread->is_internal){
            return (object)['icon_class'=> 'fa fa-sticky-note-o bg-gray', 'thread_type_text' => Lang::get('lang.posted_as_internal_note')];
        }
