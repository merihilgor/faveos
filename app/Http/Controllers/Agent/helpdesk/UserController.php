...
use App\Http\Controllers\Admin\helpdesk\AgentController;
...
class UserController extends Controller {
    ...
    {
        try {
            if (!$this->ticket_policy->accessUserProfile()) {
    ...
    public function show($id)
    {
        try {
            ...
            if (Auth::user()->id != $id) {
                if (!$this->ticket_policy->accessUserProfile() || !$checkViewPermission) {
...
// orgAutofill
    ...
    public function getProfileInfo(){
        ...
        $userInfo = User::where('id', Auth::user()->id)->
                with(['permision:permision.user_id,permision',
        ...
        ->select('id', 'first_name', 'last_name', 'user_name', 'location', 'role', 'email', 'ext','country_code','phone_number','mobile','agent_tzone','role','profile_pic','agent_tzone', 'agent_sign','internal_note','email_verify','not_accept_ticket' ,'google2fa_secret','is_2fa_enabled','google2fa_activation_date')->first()->toArray();
    ...
    public function exportUser(Request $request)
    {
        try {
            ...
            $filename = "users" . $date;
            (new ExcelController())->export($filename, $users);
        } catch (Exception $ex) {
            return redirect()->back()->with('fails', $ex->getMessage());
        }
    }
...
/**
        ...
        try {
           ...
           $userInfo = User::where('id', $userId)->
            ...
            ->select('id', 'first_name', 'last_name', 'user_name', 'location', 'role', 'email', 'ext', 'active','is_delete','ext','country_code','phone_number','mobile','agent_tzone','role','profile_pic','country_code', 'mobile', 'phone_number', 'active',
                'agent_tzone', 'agent_sign','mobile_verify','email_verify','internal_note','is_2fa_enabled', 'processing_account_disabling')->first()->toArray();
    ...
    public function createUserApi(UserRegisterRequest $request)
    {
        try {
            ...
            $phNo = ($request->work_phone) ? $request->work_phone : null;
            $user->mobile = ($request->mobile_phone) ? $request->mobile_phone : null;
    ...
    public function editUserApi($id)
    {
        try {
            ...
            $userDetails = (['id' => $user->id, 'first_name' => $user->first_name, 'last_name' => $user->last_name, 'user_name' => $user->user_name, 'email' => $user->email, 'address' => $user->internal_note, 'work_phone' => $user->phone_number, 'mobile_phone' => $user->mobile, 'country_code' => $user->country_code, 'ext' => $user->ext]);


            $organizationsData = (['organisation' => $organisations, 'organisation_department' => $orgDeptNames]);
            $response = array_merge($userDetails, $customFields, $organizationsData);
    ...
    public function updateUserApi($id, UserEditRequest $request)
    {
        try {
            ...
            $user->mobile = ($request->mobile_phone) ? $request->mobile_phone : null;
            $user->phone_number = ($request->work_phone) ? $request->work_phone : null;
            $user->internal_note = $request->address;
            $user->save();
