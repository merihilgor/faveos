...
class FilterController extends Controller
{
    ...
    public function getFilter(Request $request)
    {
        ...
        if ($request->filled('segment')) {
            ...
            if ($request->filled('api') && $request->get('api') == '1') {
                $inputs = [];
                foreach ($request->all() as $key => $value) {

                    if (!in_array($key, ['api','token','page' ,'sort-by' , 'order' , 'records_per_page','api_key'])){
    ...
    public function table()
    {
            ...
            $tickets = $ticket
                    ...
                    ->leftJoin('department as dept', 'dept.id', '=', 'tickets.dept_id')
                    ->select(
                        'tickets.id',
                        'th.title',
                        ...
                        \DB::raw('CONVERT_TZ(max(tickets.closed_at), "+00:00", "'.$this->gmt.'") as closed_at'),
                        ...
                        'tickets.is_deleted as is_deleted',
                        'tickets.closed as is_closed'
                    )->groupby('tickets.id');
        return $tickets;
    }
    ...
    public function checkRequestIsCorrect($table, $inputs)
    {
        ...
        if (count($custom_fields) > 0) {
            $table = $this->filterTableByCustomField($custom_fields, $table);
        }
    ...
    public function filterByInputs($input, $value, $table, $is_mytickets)
    {
        switch ($input) {
            ...
            case 'updated-daterange':
                ...
                break;
            case 'message':
                $table = $this->filterByKeyword($value, $table);
                return $table;
                break;

            case 'domain_name':
                $table = $this->filterByDomain($value, $table);
                return $table;
                break;
...
//                break;
    ...
    public function userIsAgent($table)
    {
        if (Auth::user()->role == 'agent') {
            ...
            if($ticket_policy->globalAccess()){
              return $table;
            } elseif ($ticket_policy->restrictedAccess()) {
              $table = $table->Where('tickets.assigned_to', '=', Auth::user()->id);
              return $table;
            } else {
                $id=Auth::user()->id;
     ...
     * @return builder
     */
    public function filterByDomain($domain_name, $table)
    {
        $domain =DB::table('satellite_helpdesk')->whereIn('name', $domain_name)->pluck('id');
        if (count($domain) == 0) {
            return $table->where('tickets.id', '=', null);
        }
        return $table->whereIn('tickets.domain_id', $domain);
    }
    ...
    {
        $table = $this->table()->leftJoin('ticket_status', 'tickets.status', '=', 'ticket_status.id');
        ...
        $table = $table->whereIn('tickets.status', getStatusArray($convert_to_array[3]));
        return $this->userIsAgent($table);
    }
    ...
    {
        $system = \App\Model\helpdesk\Settings\System::select('time_zone')->where('id', '=', 1)->first();
        $tz = \DB::table('timezone')->select('location')->where('name', '=', $system->time_zone)->first();
    ...
    public function filterByKeyword($value, $table)
    {
        $table = $table->where('th.is_internal', '=', 0);
        $table = $table->where(function ($query) use ($value) {
            foreach ($value as $term) {
                $query->orWhere("th.body", "LIKE", "%{$term}%");
            }  
        });
        return $table;
    }

    public function filterTableByCustomField($fields, $table)
    {
        $table = $table->join('ticket_form_data as tfd', function ($join) use ($fields) {
            $join->on('tfd.ticket_id', '=', 'tickets.id')
            ->where(function ($query) use ($fields){
                foreach ($fields as $key => $value) {
                    $query->orWhere([
                        ['content', 'LIKE', $value[0].'%'],
                        ['key', '=', $key]
                    ]);
                }
            });
