 ...
 */
    public function getOrgUserList(int $orgId ,Request $request)
    {
        try {
            ...
            $baseQuery = User::whereIn('id', $userId)->select('id', 'first_name', 'last_name', 'user_name','ext','phone_number',  'country_code','mobile', 'email', 'active','email_verify','mobile_verify','is_delete','updated_at')->orderBy($sortBy, $orderBy);
 ...
 */
    public function orgEditUser($id, CountryCode $code)
    {
        try {
            ...
            $users = User::whereId($id)->select('first_name','last_name','user_name','email','active','internal_note','country_code','mobile','ext','phone_number')->first();
