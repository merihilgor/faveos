                    ...
                    <div class="modal-dialog">
                        <div class="modal-content">
                            ...
                            <div class="modal-footer">
                                ...
                                <i class="fa fa-trash">&nbsp;&nbsp;</i>'.Lang::get('lang.delete').'</a>
                            </div>
                        </div>
                    </div>
                </div><script>
...
function dateTimeFormat($momentFormat = false)
{
    return Cache::remember('date_time_format_'.$momentFormat, 10, function() use($momentFormat) {
        ...
        $timeFormat = App\Model\helpdesk\Settings\System::value('time_farmat');
...
function agentTimeZone($user = null)
{
    ...
    try{
        ...
        return Cache::remember('timezone_'.$user->id,5, function() use ($user){
            ...
            if(!$timezone){
                $timezone = \App\Model\helpdesk\Settings\System::value('time_zone');
            }
        ...
        return \App\Model\helpdesk\Settings\System::value('time_zone');
    }
}
...
function getCustomFieldsOfTickets($ticketId, $columns = [], $format=false)
{
    ...
    $ticket = $ticket->customFieldValues->toArray();
    if (!$format) return $ticket;
    $formattedArray = [];
    foreach ($ticket as $value) {
        $formattedArray['custom_'.$value['form_field_id']] = $value['value'];
    }
...
function getAgentBasedOnPermission(string $permision) : array{
    ...
    $data= \App\Model\helpdesk\Agent\Permission::where('permision','!=',null)->pluck('permision','user_id')->toArray();
    $agentId = [];
    if(count($data)){
